...
resource "aws_s3_bucket" "target_bucket" {
  provider = aws.prod
  bucket   = "pl-prod-one-hop-createaccesskey-bucket-${var.account_id}-${var.resource_suffix}"
}

# Privileged user with S3 access
resource "aws_iam_user" "bucket_access_user" {
  provider = aws.prod
  name     = "pl-prod-one-hop-createaccesskey-bucket-access-user"
}
...
resource "aws_iam_user" "privesc_user" {
  provider = aws.prod
  name     = "pl-prod-one-hop-createaccesskey-bucket-privesc-user"
}
