...
resource "aws_s3_bucket" "target_bucket" {
  provider = aws.prod
  bucket   = "pl-prod-one-hop-putrolepolicy-bucket-${var.account_id}-${var.resource_suffix}"
}

# Privileged role that has S3 access
resource "aws_iam_role" "bucket_access_role" {
  provider = aws.prod
  name     = "pl-prod-one-hop-putrolepolicy-bucket-access-role"
...
# Starting role with PutRolePolicy permission on the bucket access role
resource "aws_iam_role" "privesc_role" {
  provider = aws.prod
  name     = "pl-prod-one-hop-putrolepolicy-bucket-privesc-role"
...
resource "aws_s3_object" "sensitive_file" {
  ...
  content  = "🎉 SUCCESS! You've accessed the S3 bucket via iam:PutRolePolicy privilege escalation.\n\nAttack Path:\n1. Assumed pl-prod-one-hop-putrolepolicy-bucket-privesc-role\n2. Used iam:PutRolePolicy to modify pl-prod-one-hop-putrolepolicy-bucket-access-role\n3. Added sts:AssumeRole permission to allow assuming the bucket access role\n4. Assumed the bucket access role\n5. Accessed this S3 bucket\n\nFlag: PATHFINDER-PUTROLEPOLICY-TO-BUCKET-2024"
