...
echo "DEV_ROLE_NAME=pl-dev-lambda-invoke-role"
echo "DEV_ROLE_NAME_ARN=arn:aws:iam::${DEV_ACCOUNT_ID}:role/pl-dev-lambda-invoke-role"
echo "DEV_ROLE_ARN=arn:aws:iam::${DEV_ACCOUNT_ID}:role/pl-dev-lambda-invoke-role"
echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
echo "EXISTING_PROD_LAMBDA_FUNCTION_NAME=pl-prod-hello-world-*"
echo "EXISTING_PROD_LAMBDA_FUNCTION_ARN=arn:aws:lambda:us-west-2:PROD_ACCOUNT_ID:function/pl-prod-hello-world-*"
echo ""

echo -e "${BLUE}Attack Path:${NC}"
echo "Cross-account dev → prod: Lambda function code update to extract credentials"
echo "Dev role updates prod Lambda → Invokes Lambda → Extracts prod credentials"
echo ""
