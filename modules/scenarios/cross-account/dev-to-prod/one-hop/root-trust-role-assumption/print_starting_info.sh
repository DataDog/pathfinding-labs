...
echo "STARTING_USER_DEV=pl-dev-xsarrt-to-admin-starting-user"
echo "TARGET_ROLE_PROD=pl-prod-xsarrt-to-admin-target-role"
echo "TARGET_ROLE_PROD_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-xsarrt-to-admin-target-role"
echo "TARGET_ROLE_ARN=$TARGET_ROLE_ARN"

echo ""

echo -e "${BLUE}Attack Path:${NC}"
echo "Cross-account dev → prod: Single role assumption for privilege escalation"
echo ""
