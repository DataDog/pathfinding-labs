...
TARGET_BUCKET_ARN=$(echo "$MODULE_OUTPUT" | jq -r '.target_bucket_arn')
...
echo "TARGET_BUCKET_ARN=$TARGET_BUCKET_ARN"
echo ""
echo "NOTE: This scenario contains 40 test principals (9 isAdmin users, 6 isAdmin roles, 12 notAdmin users, 12 notAdmin roles)"
echo "Run 'terraform output tool_testing_test_effective_permissions_evaluation' to see all principals"
