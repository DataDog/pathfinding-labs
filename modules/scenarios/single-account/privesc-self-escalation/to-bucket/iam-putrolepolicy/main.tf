...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-prp-to-bucket-starting-user"
...
resource "aws_iam_user_policy" "starting_user_basic" {
  provider = aws.prod
  name     = "pl-prod-prp-to-bucket-starting-user-policy"
  user     = aws_iam_user.starting_user.name
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:iam::${var.account_id}:role/pl-prod-prp-to-bucket-starting-role"
      }
    ]
  })
}
...
resource "aws_s3_bucket" "target_bucket" {
  provider = aws.prod
  bucket   = "pl-prod-prp-to-bucket-${var.account_id}-${var.resource_suffix}"
}

# Target role with S3 bucket access (will be modified via PutRolePolicy)
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-prp-to-bucket-target-role"
...
resource "aws_iam_policy" "bucket_access_policy" {
  provider    = aws.prod
  name        = "pl-prod-prp-to-bucket-access-policy"
...
resource "aws_iam_role" "starting_role" {
  provider = aws.prod
  name     = "pl-prod-prp-to-bucket-starting-role"
