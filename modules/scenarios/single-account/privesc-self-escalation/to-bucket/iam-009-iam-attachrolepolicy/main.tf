...
resource "aws_iam_user_policy" "starting_user_basic" {
  provider = aws.prod
  name     = "pl-prod-iam-009-to-bucket-starting-user-policy"
  user     = aws_iam_user.starting_user.name
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:iam::${var.account_id}:role/pl-prod-iam-009-to-bucket-starting-role"
      }
    ]
  })
}
...
resource "aws_s3_bucket" "target_bucket" {
  provider = aws.prod
  bucket   = "pl-prod-iam-009-to-bucket-${var.account_id}-${var.resource_suffix}"

  tags = {
    Name        = "pl-prod-iam-009-to-bucket-target-bucket"
    Environment = var.environment
    Scenario    = "iam-attachrolepolicy"
...
resource "aws_iam_policy" "bucket_access_policy" {
  provider    = aws.prod
  name        = "pl-prod-iam-009-to-bucket-access-policy"
