...
If the call returns a list of IAM users, escalation succeeded. The role now has effective administrator access within the account.

## Capture the Flag

Admin access isn't the finish line — the flag is. Every Pathfinding Labs scenario stores a flag in a well-known location, and retrieving it is how you prove the end-to-end attack worked. For `to-admin` scenarios like this one, the flag lives in AWS Systems Manager Parameter Store at a predictable path under `/pathfinding-labs/flags/`. Reading it requires `ssm:GetParameter` on that specific parameter, which the inline policy granting `Action: *` on `Resource: *` now provides implicitly via the starting role session.

Using the starting role session credentials (which now hold the inline administrator policy applied in the previous step), read the flag:
