...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-iam-005-to-admin-starting-user"
}
...
resource "aws_iam_user_policy" "starting_user_basic" {
  provider = aws.prod
  name     = "pl-prod-iam-005-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:iam::${var.account_id}:role/pl-prod-iam-005-to-admin-starting-role"
      }
    ]
  })
}
...
resource "aws_iam_role" "starting_role" {
  provider = aws.prod
  name     = "pl-prod-iam-005-to-admin-starting-role"
...
resource "aws_iam_policy" "privesc_policy" {
  provider    = aws.prod
  name        = "pl-prod-iam-005-to-admin-policy"
...
resource "aws_ssm_parameter" "flag" {
  ...
  tags = {
    Name        = "pl-prod-iam-005-to-admin-flag"
