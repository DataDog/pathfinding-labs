...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-cpv-to-admin-starting-user"
}
...
resource "aws_iam_user_policy" "starting_user_basic" {
  provider = aws.prod
  name     = "pl-prod-cpv-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:iam::${var.prod_account_id}:role/pl-prod-cpv-to-admin-starting-role"
      }
    ]
  })
}
...
resource "aws_iam_role" "starting_role" {
  provider = aws.prod
  name     = "pl-prod-cpv-to-admin-starting-role"
...
resource "aws_iam_policy" "privesc_policy" {
  provider    = aws.prod
  name        = "pl-prod-cpv-to-admin-policy"
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:iam::${var.prod_account_id}:policy/pl-prod-cpv-to-admin-policy"
      },
      {
        Effect = "Allow"
        Action = [
