...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_self_escalation_to_admin_iam_attachrolepolicy.value // empty')
...
echo "STARTING_USER=pl-prod-arp-to-admin-starting-user"
echo "STARTING_ROLE=pl-prod-arp-to-admin-starting-role"
echo "STARTING_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-arp-to-admin-starting-role"
