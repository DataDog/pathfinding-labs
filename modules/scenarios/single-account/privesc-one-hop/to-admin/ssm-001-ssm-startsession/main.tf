...
# Resource naming convention: pl-prod-ssm-001-to-admin-{resource-type}
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-ssm-001-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_instance_profile" "ec2_admin" {
  provider = aws.prod
  name     = "pl-prod-ssm-001-to-admin-instance-profile"
  role     = aws_iam_role.ec2_admin.name
...
resource "aws_security_group" "target_instance" {
  provider    = aws.prod
  name        = "pl-prod-ssm-001-to-admin-sg"
...
resource "aws_instance" "target" {
  ...
  tags = {
    Name        = "pl-prod-ssm-001-to-admin-target-instance"
    Environment = var.environment
    Scenario    = "ssm-startsession"
