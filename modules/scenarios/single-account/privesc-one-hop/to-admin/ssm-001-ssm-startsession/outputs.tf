...
output "target_admin_role_name" {
  description = "Name of the target admin role (for verification)"
  value       = aws_iam_role.target_admin.name
}

output "target_admin_role_arn" {
  description = "ARN of the target admin role (for verification)"
...
output "attack_path" {
  ...
  value       = "User (pl-prod-ssm-001-to-admin-starting-user) → ssm:StartSession → EC2 Instance (${aws_instance.target.id}) → Extract Instance Role Credentials via IMDS → Admin Access"
}
