...
echo "ADMIN_ROLE_NAME_ARN=arn:aws:iam::${ACCOUNT_ID}:role/${ADMIN_ROLE}"
echo "ADMIN_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/${ADMIN_ROLE}"
echo ""

echo -e "${BLUE}Attack Path:${NC}"
echo "User with iam:PassRole, lambda:CreateFunction, lambda:InvokeFunction"
echo "→ Pass admin role to Lambda → Invoke Lambda → Extract admin credentials"
echo ""
