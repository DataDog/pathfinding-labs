...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-plcflif-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-plcflif-to-admin-target-role` (Admin role passed to Lambda)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-plcflif-to-admin-starting-user] -->|iam:PassRole + lambda:CreateFunction| B[Lambda Function]
    B -->|Executes with| C[pl-prod-plcflif-to-admin-target-role]
...
1. **Initial Access**: Start as `pl-prod-plcflif-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-plcflif-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-plcflif-to-admin-target-role` | Admin role that can be passed to Lambda functions |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole+lambda-createfunction+lambda-invokefunction
./demo_attack.sh
```
