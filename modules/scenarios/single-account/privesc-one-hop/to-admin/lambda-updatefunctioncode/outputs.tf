...
output "attack_path" {
  ...
  value       = "User (pl-prod-lufc-to-admin-starting-user) → UpdateFunctionCode → Lambda Function (pl-prod-lufc-to-admin-target-lambda) → InvokeFunction → Executes with Admin Role (pl-prod-lufc-to-admin-target-role) → Admin Access"
}
