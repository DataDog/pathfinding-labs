...
echo "STARTING_USER=pl-prod-glue-005-to-admin-starting-user"
echo "TARGET_ROLE=pl-prod-glue-005-to-admin-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-glue-005-to-admin-target-role"
echo "INITIAL_ROLE=pl-prod-glue-005-to-admin-initial-role"
echo "INITIAL_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-glue-005-to-admin-initial-role"

echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
# Extract Glue Job name from terraform output
GLUE_JOB_NAME=$(echo "$MODULE_OUTPUT" | jq -r '.glue_job_name // "unknown"')
echo "EXISTING_GLUE_JOB_NAME=$GLUE_JOB_NAME"
if [ "$GLUE_JOB_NAME" != "unknown" ]; then
  echo "EXISTING_GLUE_JOB_ARN=arn:aws:glue:${AWS_REGION}:${ACCOUNT_ID}:job/${GLUE_JOB_NAME}"
fi
