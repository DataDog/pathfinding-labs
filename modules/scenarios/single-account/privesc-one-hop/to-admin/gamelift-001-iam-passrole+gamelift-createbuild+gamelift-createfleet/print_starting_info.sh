...
# Configuration
ADMIN_ROLE="pl-prod-gamelift-001-to-admin-admin-role"
...
echo "ADMIN_ROLE_NAME=$ADMIN_ROLE"
echo "ADMIN_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/${ADMIN_ROLE}"
echo ""

echo -e "${BLUE}Attack Path:${NC}"
echo "User with iam:PassRole, gamelift:CreateBuild, gamelift:RequestUploadCredentials, gamelift:CreateFleet"
echo "→ Upload malicious game server build → Create fleet with admin instance role (SHARED_CREDENTIAL_FILE)"
echo "→ Game server reads shared credentials → iam:AttachUserPolicy → Admin access"
echo ""
