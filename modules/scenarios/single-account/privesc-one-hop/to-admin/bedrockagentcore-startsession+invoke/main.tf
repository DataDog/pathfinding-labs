...
#
# Resource naming convention: pl-prod-baci-to-admin-{resource-type}
# Scenario shorthand: "baci" (Bedrock AgentCore Code Interpreter)

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-baci-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-baci-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-baci-to-admin-target-role"
...
resource "aws_bedrockagentcore_code_interpreter" "existing_interpreter" {
  provider = aws.prod
  name     = "pl_prod_baci_to_admin_target_interpreter"
  ...
  tags = {
    Name        = "pl-prod-baci-to-admin-target-interpreter"
    Environment = var.environment
    Scenario    = "bedrockagentcore-startsession+invoke"
