...
output "attack_path" {
  ...
  value       = "User (pl-prod-prcp-to-admin-starting-user) → Upload malicious script to S3 → CreateProcessingJob with admin role (pl-prod-prcp-to-admin-passable-role) → Processing job executes script with admin privileges → Script grants admin access to starting user"
}
