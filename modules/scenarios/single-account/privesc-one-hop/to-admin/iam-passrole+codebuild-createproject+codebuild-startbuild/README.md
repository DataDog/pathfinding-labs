...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cbcpsb-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cbcpsb-to-admin-target-role` (Privileged role passed to CodeBuild)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-cbcpsb-to-admin-starting-user] -->|codebuild:CreateProject + iam:PassRole| B[CodeBuild Project]
    B -->|Service assumes role| C[pl-prod-cbcpsb-to-admin-target-role]
...
1. **Initial Access**: Start as `pl-prod-cbcpsb-to-admin-starting-user` (credentials provided via Terraform outputs)
2. **Create CodeBuild Project**: Use `codebuild:CreateProject` to create a new project, passing the privileged `pl-prod-cbcpsb-to-admin-target-role` via `iam:PassRole`
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cbcpsb-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cbcpsb-to-admin-target-role` | Privileged role with iam:AttachUserPolicy permission, trusted by CodeBuild service |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-cbcpsb-to-admin-user-policy` | Policy granting codebuild:CreateProject, codebuild:StartBuild, and iam:PassRole to starting user |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole+codebuild-createproject+codebuild-startbuild
./demo_attack.sh
```
