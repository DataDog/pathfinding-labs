...
# Resource naming convention: pl-prod-prcuss-to-admin-{resource-type}
# prcuss = PassRole CloudFormation UpdateStackSet
# All resources use provider = aws.prod

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-prcuss-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-prcuss-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:iam::${var.account_id}:role/pl-prod-prcuss-to-admin-stackset-admin-role"
      },
      {
        ...
        Resource = "arn:aws:cloudformation:*:${var.account_id}:stackset/pl-prod-prcuss-to-admin-stackset:*"
      },
      {
      ...
      },
      {
        ...
        Resource = "arn:aws:iam::${var.account_id}:role/pl-prod-prcuss-to-admin-escalated-role"
      },
      {
...
resource "aws_iam_role" "stackset_execution_role" {
  provider = aws.prod
  name     = "pl-prod-prcuss-to-admin-stackset-execution-role"
...
resource "aws_iam_role" "stackset_admin_role" {
  provider = aws.prod
  name     = "pl-prod-prcuss-to-admin-stackset-admin-role"
...
resource "aws_iam_role_policy" "stackset_admin_role_policy" {
  provider = aws.prod
  name     = "pl-prod-prcuss-to-admin-stackset-admin-policy"
  ...
  template_body = jsonencode({
    ...
    Resources = {
      BenignBucket = {
        Type = "AWS::S3::Bucket"
        Properties = {
          BucketName = "pl-prod-prcuss-benign-${var.account_id}-${var.resource_suffix}"
          Tags = [
            {
              Key   = "Name"
