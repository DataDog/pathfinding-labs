...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prcuss-to-admin-starting-user` (Scenario-specific starting user with UpdateStackSet permission)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prcuss-to-admin-stackset-execution-role` (StackSet execution role with administrative permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prcuss-to-admin-escalated-role` (New admin role created via StackSet update)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-prcuss-to-admin-starting-user] -->|cloudformation:UpdateStackSet| B[CloudFormation StackSet]
    B -->|Uses execution role| C[pl-prod-prcuss-to-admin-stackset-execution-role]
    C -->|iam:CreateRole| D[pl-prod-prcuss-to-admin-escalated-role]
    A -->|sts:AssumeRole| D
...
1. **Initial Access**: Start as `pl-prod-prcuss-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prcuss-to-admin-starting-user` | Scenario-specific starting user with access keys and cloudformation:UpdateStackSet permission |
| `arn:aws:cloudformation:*:PROD_ACCOUNT:stackset/pl-prod-prcuss-to-admin-stackset:*` | CloudFormation StackSet with administrative execution role |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prcuss-to-admin-stackset-execution-role` | StackSet execution role with AdministratorAccess policy |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prcuss-to-admin-escalated-role` | Admin role created during StackSet update (created by demo script) |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole-cloudformation-updatestackset
./demo_attack.sh
```
