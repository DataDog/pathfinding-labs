...
output "escalated_role_name" {
  ...
  value       = "pl-prod-prcuss-to-admin-escalated-role"
}

# Attack path description
output "attack_path" {
  description = "Description of the attack path"
  value       = "User (pl-prod-prcuss-to-admin-starting-user) → iam:PassRole + cloudformation:UpdateStackSet → StackSet (pl-prod-prcuss-to-admin-stackset) with admin execution role → creates escalated admin role (pl-prod-prcuss-to-admin-escalated-role) → sts:AssumeRole → admin access"
}
