...
output "attack_path" {
  ...
  value       = "User (pl-prod-gcj-ct-to-admin-starting-user) → iam:PassRole + glue:CreateJob → Create Glue job with admin role → glue:CreateTrigger (--start-on-creation) → Job executes and attaches AdministratorAccess to starting user → Admin access"
}
