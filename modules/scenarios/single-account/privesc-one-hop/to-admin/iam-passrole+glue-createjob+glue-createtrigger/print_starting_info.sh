...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_iam_passrole_glue_createjob_glue_createtrigger.value // empty')
...
echo "STARTING_USER=pl-prod-gcj-ct-to-admin-starting-user"
echo "TARGET_ROLE=pl-prod-gcj-ct-to-admin-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-gcj-ct-to-admin-target-role"
