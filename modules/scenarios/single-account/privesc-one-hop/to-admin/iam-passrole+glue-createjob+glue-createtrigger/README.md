...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-gcj-ct-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-gcj-ct-to-admin-target-role` (Admin role passed to Glue job)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-gcj-ct-to-admin-starting-user] -->|iam:PassRole + glue:CreateJob| B[Glue Job with Admin Role]
...
1. **Initial Access**: Start as `pl-prod-gcj-ct-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-gcj-ct-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-gcj-ct-to-admin-target-role` | Administrative role passed to Glue job |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-gcj-ct-to-admin-passrole-policy` | Policy allowing PassRole on target role, glue:CreateJob, and glue:CreateTrigger |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole+glue-createjob+glue-createtrigger
./demo_attack.sh
```
