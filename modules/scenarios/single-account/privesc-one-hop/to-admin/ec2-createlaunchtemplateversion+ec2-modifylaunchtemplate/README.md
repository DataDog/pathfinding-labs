...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-lt-modify-to-admin-starting-user` (Scenario-specific starting user with template modification permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-lt-modify-to-admin-lowpriv-role` (Low-privilege role in original launch template)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-lt-modify-to-admin-target-role` (Administrative role passed to modified launch template)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-lt-modify-to-admin-starting-user] -->|CreateLaunchTemplateVersion| B[New Template Version]
...
1. **Initial Access**: Start as `pl-prod-lt-modify-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-lt-modify-to-admin-starting-user` | Scenario-specific starting user with access keys and template modification permissions |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-lt-modify-to-admin-lowpriv-role` | Low-privilege role initially configured in the launch template |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-lt-modify-to-admin-target-role` | Administrative role that will be passed to the modified launch template |
| `arn:aws:ec2:REGION:PROD_ACCOUNT:launch-template/pl-prod-lt-modify-to-admin-template` | EC2 launch template that can be modified to include admin role |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/ec2-createlaunchtemplateversion+ec2-modifylaunchtemplate
./demo_attack.sh
```
