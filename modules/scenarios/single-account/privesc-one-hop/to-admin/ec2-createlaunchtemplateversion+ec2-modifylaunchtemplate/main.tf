...
# Resource naming convention: pl-prod-lt-modify-to-admin-{resource-type}
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-lt-modify-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-lt-modify-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "lowpriv_role" {
  provider = aws.prod
  name     = "pl-prod-lt-modify-to-admin-lowpriv-role"
...
resource "aws_iam_role_policy" "lowpriv_role_policy" {
  provider = aws.prod
  name     = "pl-prod-lt-modify-to-admin-lowpriv-policy"
...
resource "aws_iam_instance_profile" "lowpriv_profile" {
  provider = aws.prod
  name     = "pl-prod-lt-modify-to-admin-lowpriv-profile"
...
resource "aws_iam_role" "target_admin_role" {
  provider = aws.prod
  name     = "pl-prod-lt-modify-to-admin-target-role"
...
resource "aws_iam_instance_profile" "target_admin_profile" {
  provider = aws.prod
  name     = "pl-prod-lt-modify-to-admin-target-profile"
  role     = aws_iam_role.target_admin_role.name
...
resource "aws_security_group" "victim_sg" {
  provider    = aws.prod
  name        = "pl-prod-lt-modify-to-admin-victim-sg"
...
resource "aws_launch_template" "victim_template" {
  provider      = aws.prod
  name          = "pl-prod-lt-modify-to-admin-victim-template"
  ...
  tag_specifications {
    ...
    tags = {
      Name        = "pl-prod-lt-modify-to-admin-victim-instance"
      Environment = var.environment
      Scenario    = "ec2-createlaunchtemplateversion+ec2-modifylaunchtemplate"
    }
  }
...
resource "aws_autoscaling_group" "victim_asg" {
  provider            = aws.prod
  name                = "pl-prod-lt-modify-to-admin-victim-asg"
  ...
  tag {
    key                 = "Name"
    value               = "pl-prod-lt-modify-to-admin-victim-asg"
