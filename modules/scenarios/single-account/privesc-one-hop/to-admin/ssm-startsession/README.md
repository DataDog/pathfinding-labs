...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-sss-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:ec2:REGION:PROD_ACCOUNT:instance/i-xxxxxxxxx` (EC2 instance with SSM agent)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-sss-to-admin-ec2-role` (Administrative role attached to EC2 instance)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-sss-to-admin-starting-user] -->|ssm:StartSession| B[EC2 Instance]
...
1. **Initial Access**: Start as `pl-prod-sss-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-sss-to-admin-starting-user` | Scenario-specific starting user with access keys and SSM permissions |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-sss-to-admin-policy` | Allows `ssm:StartSession`, `ssm:TerminateSession`, `ssm:ResumeSession`, `ssm:DescribeInstanceInformation`, and `ec2:DescribeInstances` |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-sss-to-admin-ec2-role` | Administrative role attached to the EC2 instance (target for credential extraction) |
| `arn:aws:iam::PROD_ACCOUNT:instance-profile/pl-prod-sss-to-admin-ec2-profile` | Instance profile associating the admin role with the EC2 instance |
