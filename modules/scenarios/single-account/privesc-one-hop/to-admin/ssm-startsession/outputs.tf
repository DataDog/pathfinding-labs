...
output "attack_path" {
  ...
  value       = "User (pl-prod-sss-to-admin-starting-user) → ssm:StartSession → EC2 Instance (${aws_instance.target.id}) → Extract Instance Role Credentials via IMDS → Admin Access"
}
