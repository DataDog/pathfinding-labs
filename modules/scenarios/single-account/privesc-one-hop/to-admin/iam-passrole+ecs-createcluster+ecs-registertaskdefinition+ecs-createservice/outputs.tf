...
output "attack_path" {
  ...
  value       = "User (pl-prod-precccs-to-admin-starting-user) → ecs:CreateCluster → ecs:RegisterTaskDefinition (with ${aws_iam_role.target_role.name}) → ecs:CreateService (on Fargate) → ECS task launches container that attaches AdministratorAccess to starting user → admin access"
}
