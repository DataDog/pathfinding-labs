...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-precccs-to-admin-starting-user` (Scenario-specific starting user with ECS and PassRole permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-precccs-to-admin-target-role` (Target role with administrative permissions that can be passed to ECS tasks)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-precccs-to-admin-starting-user] -->|ecs:CreateCluster| B[ECS Cluster Created]
...
1. **Initial Access**: Start as `pl-prod-precccs-to-admin-starting-user` (credentials provided via Terraform outputs)
...
3. **Register Task Definition**: Use `ecs:RegisterTaskDefinition` and `iam:PassRole` to create a task definition that:
   - Uses the privileged `pl-prod-precccs-to-admin-target-role` as the task execution role
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-precccs-to-admin-starting-user` | Scenario-specific starting user with access keys and ECS permissions |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-precccs-to-admin-target-role` | Target role with AdministratorAccess policy that can be passed to ECS tasks |
