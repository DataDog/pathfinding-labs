...
output "attack_path" {
  ...
  value       = "User (pl-prod-cfcs-to-admin-starting-user) → cloudformation:CreateChangeSet + ExecuteChangeSet on stack (pl-prod-cfcs-to-admin-target-stack) → stack updates using admin service role (pl-prod-cfcs-to-admin-stack-role) → creates escalated-role with admin access → admin privileges"
}
