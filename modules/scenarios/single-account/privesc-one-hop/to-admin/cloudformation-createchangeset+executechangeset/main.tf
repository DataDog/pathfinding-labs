...
# Resource naming convention: pl-prod-cfcs-to-admin-{resource-type}
# All resources use provider = aws.prod (single account scenario)

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-cfcs-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-cfcs-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:iam::${var.account_id}:role/pl-prod-cfcs-to-admin-escalated-role"
      }
    ]
  })
}
...
resource "aws_iam_role" "stack_role" {
  provider = aws.prod
  name     = "pl-prod-cfcs-to-admin-stack-role"
...
resource "aws_cloudformation_stack" "target_stack" {
  provider = aws.prod
  name     = "pl-prod-cfcs-to-admin-target-stack"
  ...
  template_body = jsonencode({
    ...
    Resources = {
      InitialBucket = {
        Type = "AWS::S3::Bucket"
        Properties = {
          BucketName = "pl-prod-cfcs-to-admin-initial-bucket-${var.account_id}-${var.resource_suffix}"
          Tags = [
            {
              Key   = "Name"
