...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-iam-003-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-iam-003-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
  ...
  policy = jsonencode({
    ...
    Statement = [
      {
        Sid    = "RequiredForExploitationDeleteAndCreateAccessKey"
        Effect = "Allow"
        Action = [
          "iam:DeleteAccessKey",
          "iam:CreateAccessKey"
...
resource "aws_iam_user" "admin_user" {
  provider = aws.prod
  name     = "pl-prod-iam-003-to-admin-target-user"
...
resource "aws_ssm_parameter" "flag" {
  ...
  tags = {
    Name        = "pl-prod-iam-003-to-admin-flag"
    Environment = var.environment
    Scenario    = "iam-deleteaccesskey+createaccesskey"
    Purpose     = "ctf-flag"
  }
