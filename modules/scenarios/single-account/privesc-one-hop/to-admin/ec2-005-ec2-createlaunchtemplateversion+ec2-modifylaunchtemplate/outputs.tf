...
output "attack_path" {
  ...
  value       = "User (pl-prod-ec2-005-to-admin-starting-user) → CreateLaunchTemplateVersion (with admin role + malicious user data) → ModifyLaunchTemplate (set default version) → Next instance launch uses admin role → User data grants admin access to starting user"
}
