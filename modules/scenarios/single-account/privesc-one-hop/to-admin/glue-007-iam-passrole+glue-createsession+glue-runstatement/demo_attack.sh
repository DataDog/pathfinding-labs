...
if [ $? -eq 0 ]; then
    echo -e "${GREEN}OK Successfully created Glue Interactive Session with admin role!${NC}"
else
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    ...
    if [ "$SESSION_STATUS" = "READY" ]; then
        echo -e "${GREEN}OK Glue Interactive Session is ready!${NC}\n"
        break
    elif [ "$SESSION_STATUS" = "FAILED" ] || [ "$SESSION_STATUS" = "STOPPED" ] || [ "$SESSION_STATUS" = "TIMEOUT" ]; then
        echo -e "${RED}X Glue session failed with status: $SESSION_STATUS${NC}"
...
if [ $ELAPSED -ge $MAX_WAIT ]; then
    echo -e "${RED}X Timeout: Session did not become ready within expected time${NC}"
    exit 1
fi
...
echo -e "${GREEN}OK Statement submitted successfully${NC}\n"
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    ...
    if [ "$STATEMENT_STATUS" = "AVAILABLE" ]; then
        echo -e "${GREEN}OK Statement completed successfully!${NC}"
        ...
        echo -e "${RED}X Statement failed with status: $STATEMENT_STATUS${NC}"
        echo "Fetching error details..."
...
if [ $ELAPSED -ge $MAX_WAIT ]; then
    echo -e "${RED}X Timeout: Statement did not complete within expected time${NC}"
    exit 1
fi
...
sleep 15
echo -e "${GREEN}OK Policy propagation complete${NC}\n"
...
if aws iam list-users --max-items 3 --output table; then
    ...
    echo -e "${RED}X Failed to list users${NC}"
    exit 1
fi
echo ""
