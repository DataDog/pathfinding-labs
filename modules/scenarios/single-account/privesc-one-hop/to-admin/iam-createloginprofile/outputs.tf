...
output "attack_path" {
  ...
  value       = "User (pl-prod-clp-to-admin-starting-user) → Role (pl-prod-clp-to-admin-starting-role) → CreateLoginProfile → Admin User (pl-prod-clp-to-admin-target-user) → Console Access with Admin Privileges"
}
