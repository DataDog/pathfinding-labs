...
# Resource naming convention: pl-prod-cbsbb-to-admin-{resource-type}

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-cbsbb-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-cbsbb-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-cbsbb-to-admin-target-role"
...
resource "aws_iam_role_policy" "target_role_policy" {
  provider = aws.prod
  name     = "pl-prod-cbsbb-to-admin-target-role-policy"
...
resource "aws_codebuild_project" "target_project" {
  provider = aws.prod
  name     = "pl-prod-cbsbb-to-admin-target-project"
