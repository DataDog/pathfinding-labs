...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_codebuild_startbuildbatch.value // empty')
...
echo "STARTING_USER=pl-prod-cbsbb-to-admin-starting-user"
echo "TARGET_ROLE=pl-prod-cbsbb-to-admin-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-cbsbb-to-admin-target-role"

echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
echo "EXISTING_CODEBUILD_PROJECT_NAME=pl-prod-cbsbb-to-admin-target-project"
echo "EXISTING_CODEBUILD_PROJECT_ARN=arn:aws:codebuild:${AWS_REGION}:${ACCOUNT_ID}:project/pl-prod-cbsbb-to-admin-target-project"
