...
output "attack_path" {
  ...
  value       = "User (pl-prod-cbsbb-to-admin-starting-user) → codebuild:StartBuildBatch with buildspec-override → existing CodeBuild project (pl-prod-cbsbb-to-admin-target-project) → buildspec executes with admin role → grants admin to starting_user → admin access"
}
