...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cbsbb-to-admin-starting-user` (Scenario-specific starting user with StartBuildBatch permission)
- `arn:aws:codebuild:REGION:PROD_ACCOUNT:project/pl-prod-cbsbb-to-admin-target-project` (Existing CodeBuild project with admin service role)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cbsbb-to-admin-target-role` (Service role with iam:AttachUserPolicy permission, trusted by CodeBuild)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-cbsbb-to-admin-starting-user] -->|codebuild:ListProjects| B[Discovery]
    B -->|codebuild:BatchGetProjects| C[pl-prod-cbsbb-to-admin-target-project]
    C -->|codebuild:StartBuildBatch<br/>with --buildspec-override| D[Malicious Buildspec Execution]
    D -->|Executes with| E[pl-prod-cbsbb-to-admin-target-role]
...
1. **Initial Access**: Start as `pl-prod-cbsbb-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cbsbb-to-admin-starting-user` | Scenario-specific starting user with access keys and codebuild:StartBuildBatch permission |
| `arn:aws:codebuild:REGION:PROD_ACCOUNT:project/pl-prod-cbsbb-to-admin-target-project` | Existing CodeBuild project configured for batch builds |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cbsbb-to-admin-target-role` | Service role with iam:AttachUserPolicy permission, attached to CodeBuild project |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-cbsbb-to-admin-starting-user-policy` | Grants codebuild:StartBuildBatch, ListProjects, BatchGetProjects, and BatchGetBuildBatches permissions |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-cbsbb-to-admin-target-role-policy` | Grants iam:AttachUserPolicy permission to the service role |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/codebuild-startbuildbatch
./demo_attack.sh
```
