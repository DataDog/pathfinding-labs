...
* **CTF Flag Location:** ssm-parameter `/pathfinding-labs/flags/ec2-002-to-admin`
...
2. Navigate to `ec2-002-to-admin` in the scenarios list
...
| `arn:aws:ssm:{region}:{account_id}:parameter/pathfinding-labs/flags/ec2-002-to-admin` | CTF flag stored in SSM Parameter Store; retrievable by any admin-equivalent principal |
...
```bash
plabs disable ec2-002-to-admin
plabs apply
```
