...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-arpsar-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-arpsar-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:iam::${var.account_id}:role/pl-prod-arpsar-to-admin-target-role"
      },
      {
        Effect = "Allow"
        Action = [
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-arpsar-to-admin-target-role"
...
resource "aws_iam_role_policy" "target_role_minimal_policy" {
  provider = aws.prod
  name     = "pl-prod-arpsar-to-admin-minimal-policy"
