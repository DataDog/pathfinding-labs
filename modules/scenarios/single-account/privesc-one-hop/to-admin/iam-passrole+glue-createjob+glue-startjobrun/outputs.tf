...
output "attack_path" {
  ...
  value       = "User (pl-prod-gcj-sjr-to-admin-starting-user) → iam:PassRole + glue:CreateJob → Create Glue Job with admin role → glue:StartJobRun → Job executes Python script to attach AdministratorAccess policy to starting user → admin access"
}
