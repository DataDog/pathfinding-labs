...
# Resource naming convention: pl-prod-gud-to-admin-{resource-type}
# All resources use provider = aws.prod

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-gud-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-gud-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-gud-to-admin-target-role"
...
resource "aws_glue_dev_endpoint" "target_endpoint" {
  provider = aws.prod
  name     = "pl-prod-gud-to-admin-endpoint"
