...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-gud-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:glue:REGION:PROD_ACCOUNT:devEndpoint/pl-prod-gud-to-admin-endpoint` (Pre-existing Glue dev endpoint)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-gud-to-admin-target-role` (Administrative role attached to the endpoint)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-gud-to-admin-starting-user] -->|glue:UpdateDevEndpoint| B[Add SSH Public Key]
    B -->|SSH Connection| C[pl-prod-gud-to-admin-endpoint]
    C -->|Execute with role| D[pl-prod-gud-to-admin-target-role]
...
1. **Initial Access**: Start as `pl-prod-gud-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-gud-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-gud-to-admin-policy` | Allows `glue:UpdateDevEndpoint` and `glue:GetDevEndpoint` permissions |
| `arn:aws:glue:REGION:PROD_ACCOUNT:devEndpoint/pl-prod-gud-to-admin-endpoint` | Pre-existing Glue development endpoint with administrative role |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-gud-to-admin-target-role` | Administrative role attached to the Glue dev endpoint |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-gud-to-admin-endpoint-service-role` | Service role allowing Glue to assume the target role |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/glue-updatedevendpoint
./demo_attack.sh
```
