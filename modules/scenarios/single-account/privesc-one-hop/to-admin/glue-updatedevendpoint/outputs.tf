...
output "attack_path" {
  ...
  value       = "User (pl-prod-gud-to-admin-starting-user) → glue:UpdateDevEndpoint → Add SSH key to existing dev endpoint → SSH access → Execute commands with admin role (pl-prod-gud-to-admin-target-role)"
}
