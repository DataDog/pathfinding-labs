...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-aupcak-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-aupcak-to-admin-target-user` (Target user that will be granted admin access)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-aupcak-to-admin-starting-user] -->|iam:AttachUserPolicy| B[pl-prod-aupcak-to-admin-target-user]
...
1. **Initial Access**: Start as `pl-prod-aupcak-to-admin-starting-user` (credentials provided via Terraform outputs)
2. **Attach Admin Policy**: Use `iam:AttachUserPolicy` to attach the AWS-managed `AdministratorAccess` policy to `pl-prod-aupcak-to-admin-target-user`
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-aupcak-to-admin-starting-user` | Scenario-specific starting user with access keys and permissions to attach policies and create keys for target user |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-aupcak-to-admin-target-user` | Target user that will be granted admin access via policy attachment |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-aupcak-to-admin-starting-user-policy` | IAM policy granting AttachUserPolicy and CreateAccessKey on target user |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-attachuserpolicy+iam-createaccesskey
./demo_attack.sh
```
