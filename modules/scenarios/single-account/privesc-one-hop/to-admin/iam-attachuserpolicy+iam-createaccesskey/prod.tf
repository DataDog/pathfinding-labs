...
# Resource naming convention: pl-prod-aupcak-to-admin-{resource-type}
# All resources use provider = aws.prod

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-aupcak-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-aupcak-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_user" "target_user" {
  provider = aws.prod
  name     = "pl-prod-aupcak-to-admin-target-user"
...
resource "aws_iam_user_policy" "target_user_initial_policy" {
  provider = aws.prod
  name     = "pl-prod-aupcak-to-admin-target-user-initial-policy"
