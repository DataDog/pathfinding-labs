...
output "attack_path" {
  ...
  value       = "User (pl-prod-aupcak-to-admin-starting-user) → AttachUserPolicy (attach AdministratorAccess to target_user) → CreateAccessKey (for target_user) → authenticate as target_user → Admin access"
}
