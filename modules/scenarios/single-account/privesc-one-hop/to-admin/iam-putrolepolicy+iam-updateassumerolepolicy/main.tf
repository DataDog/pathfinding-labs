...
# Resource naming convention: pl-prod-prpuar-to-admin-{resource-type}
# prpuar = PutRolePolicy + UpdateAssumeRolePolicy

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-prpuar-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-prpuar-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-prpuar-to-admin-target-role"
