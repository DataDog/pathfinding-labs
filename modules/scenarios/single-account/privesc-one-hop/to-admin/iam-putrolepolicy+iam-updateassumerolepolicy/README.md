...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prpuar-to-admin-starting-user` (Scenario-specific starting user with iam:PutRolePolicy and iam:UpdateAssumeRolePolicy permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prpuar-to-admin-target-role` (Target role with minimal initial permissions)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-prpuar-to-admin-starting-user] -->|iam:PutRolePolicy| B[pl-prod-prpuar-to-admin-target-role]
...
1. **Initial Access**: Start as `pl-prod-prpuar-to-admin-starting-user` (credentials provided via Terraform outputs)
2. **Add Admin Policy**: Use `iam:PutRolePolicy` to attach an inline policy granting `AdministratorAccess` to `pl-prod-prpuar-to-admin-target-role`
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prpuar-to-admin-starting-user` | Scenario-specific starting user with access keys and permissions to modify target role |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prpuar-to-admin-target-role` | Target role with minimal initial permissions that can be escalated |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-prpuar-to-admin-starting-user-policy` | Inline policy granting iam:PutRolePolicy and iam:UpdateAssumeRolePolicy on target role |
