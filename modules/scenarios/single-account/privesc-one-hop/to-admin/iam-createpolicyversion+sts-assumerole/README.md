...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cpvsar-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-cpvsar-to-admin-target-policy` (Customer-managed policy that can be versioned)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cpvsar-to-admin-target-role` (Target role with policy attached)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-cpvsar-to-admin-starting-user] -->|iam:CreatePolicyVersion| B[pl-prod-cpvsar-to-admin-target-policy v2]
    B -->|Policy attached to| C[pl-prod-cpvsar-to-admin-target-role]
    A -->|sts:AssumeRole| C
...
1. **Initial Access**: Start as `pl-prod-cpvsar-to-admin-starting-user` (credentials provided via Terraform outputs)
2. **Policy Reconnaissance**: Discover the customer-managed policy `pl-prod-cpvsar-to-admin-target-policy` and verify it's attached to a role
...
5. **Assume Role**: Assume the target role `pl-prod-cpvsar-to-admin-target-role` which now has admin permissions via the modified policy
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cpvsar-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-cpvsar-to-admin-target-policy` | Customer-managed policy with initial non-privileged permissions |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cpvsar-to-admin-target-role` | Target role with the customer-managed policy attached and trust policy allowing starting user to assume it |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-createpolicyversion+sts-assumerole
./demo_attack.sh
```
