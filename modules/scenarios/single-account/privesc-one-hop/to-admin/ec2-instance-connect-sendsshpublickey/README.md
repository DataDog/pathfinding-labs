...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-eic-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:ec2:REGION:PROD_ACCOUNT:instance/i-xxxxxxxxx` (EC2 instance with admin role attached)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-eic-to-admin-ec2-admin-role` (Admin role attached to EC2 instance)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-eic-to-admin-starting-user] -->|ec2-instance-connect:SendSSHPublicKey| B[EC2 Instance]
...
1. **Initial Access**: Start as `pl-prod-eic-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-eic-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-eic-to-admin-policy` | Allows `ec2-instance-connect:SendSSHPublicKey`, `ec2:DescribeInstances`, and read-only IAM discovery |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-eic-to-admin-ec2-admin-role` | Admin role attached to the EC2 instance profile |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/ec2-instance-connect-sendsshpublickey
./demo_attack.sh
```
