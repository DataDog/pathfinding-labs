...
output "escalated_role_name" {
  ...
  value       = "pl-prod-pcfss-to-admin-escalated-role"
}

output "attack_path" {
  description = "Description of the attack path"
  value       = "User (pl-prod-pcfss-to-admin-starting-user) → PassRole + CreateStackSet → StackSet passes execution role (pl-prod-pcfss-to-admin-execution-role) → CloudFormation creates escalated role (pl-prod-pcfss-to-admin-escalated-role) with admin access → Assume escalated role → Admin access"
}
