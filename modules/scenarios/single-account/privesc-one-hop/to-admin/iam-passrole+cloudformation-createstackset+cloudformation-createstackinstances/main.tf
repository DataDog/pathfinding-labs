...
# Resource naming convention: pl-prod-pcfss-to-admin-{resource-type}
# pcfss = PassRole CloudFormation StackSet

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-pcfss-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-pcfss-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:iam::${var.account_id}:role/pl-prod-pcfss-to-admin-escalated-role"
      },
      {
...
resource "aws_iam_role" "execution_role" {
  provider = aws.prod
  name     = "pl-prod-pcfss-to-admin-execution-role"
...
resource "aws_iam_role" "administration_role" {
  provider = aws.prod
  name     = "pl-prod-pcfss-to-admin-admin-role"
...
resource "aws_iam_role_policy" "administration_role_policy" {
  provider = aws.prod
  name     = "pl-prod-pcfss-to-admin-admin-role-policy"
