...
# Resource naming convention: pl-prod-prct-to-admin-{resource-type}
# prct = PassRole + CreateTrainingJob

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-prct-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-prct-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "passable_admin_role" {
  provider = aws.prod
  name     = "pl-prod-prct-to-admin-passable-role"
...
resource "aws_s3_bucket" "training_bucket" {
  provider = aws.prod
  bucket   = "pl-prod-prct-to-admin-bucket-${var.account_id}-${var.resource_suffix}"

  tags = {
    Name        = "pl-prod-prct-to-admin-bucket"
    Environment = var.environment
    Scenario    = "iam-passrole+sagemaker-createtrainingjob"
