...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prct-to-admin-starting-user` (Scenario-specific starting user with limited permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prct-to-admin-passable-role` (Admin role that can be passed to SageMaker)
- `arn:aws:s3:::pl-prod-prct-to-admin-bucket-PROD_ACCOUNT-SUFFIX` (S3 bucket for training script storage)
- `arn:aws:sagemaker:REGION:PROD_ACCOUNT:training-job/pl-prod-prct-to-admin-training-job` (SageMaker training job executing with admin privileges)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-prct-to-admin-starting-user] -->|s3:PutObject| B[Upload Malicious Script to S3]
    B -->|iam:PassRole + sagemaker:CreateTrainingJob| C[SageMaker Training Job]
    C -->|Executes with| D[pl-prod-prct-to-admin-passable-role]
...
1. **Initial Access**: Start as `pl-prod-prct-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prct-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prct-to-admin-passable-role` | Admin role that trusts SageMaker service and can be passed to training jobs |
| `arn:aws:s3:::pl-prod-prct-to-admin-bucket-PROD_ACCOUNT-SUFFIX` | S3 bucket for storing training scripts and outputs |
