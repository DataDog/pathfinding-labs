...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prcn-to-admin-starting-user` (Scenario-specific starting user with PassRole and CreateNotebookInstance permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prcn-to-admin-passable-role` (Admin role passed to SageMaker notebook instance)
- `arn:aws:sagemaker:REGION:PROD_ACCOUNT:notebook-instance/pl-prod-prcn-to-admin-notebook` (Attacker-created notebook instance with admin role)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-prcn-to-admin-starting-user] -->|iam:PassRole + sagemaker:CreateNotebookInstance| B[SageMaker Notebook Instance]
    B -->|Executes with| C[pl-prod-prcn-to-admin-passable-role]
...
1. **Initial Access**: Start as `pl-prod-prcn-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prcn-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prcn-to-admin-passable-role` | Admin role that can be passed to SageMaker notebook instances (trusted by sagemaker.amazonaws.com) |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole+sagemaker-createnotebookinstance
./demo_attack.sh
```
