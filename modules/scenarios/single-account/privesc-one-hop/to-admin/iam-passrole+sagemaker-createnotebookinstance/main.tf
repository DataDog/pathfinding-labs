...
# Resource naming convention: pl-prod-prcn-to-admin-{resource-type}
# For single account scenarios, use provider = aws.prod

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-prcn-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-prcn-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "passable_role" {
  provider = aws.prod
  name     = "pl-prod-prcn-to-admin-passable-role"
