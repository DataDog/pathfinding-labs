...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_iam_putuserpolicy_iam_createaccesskey.value // empty')
...
echo "STARTING_USER=pl-prod-pupcak-to-admin-starting-user"
echo "TARGET_USER=pl-prod-pupcak-to-admin-target-user"
echo "TARGET_USER_ARN=arn:aws:iam::${ACCOUNT_ID}:user/pl-prod-pupcak-to-admin-target-user"
echo "ADMIN_POLICY_NAME=admin-escalation"
