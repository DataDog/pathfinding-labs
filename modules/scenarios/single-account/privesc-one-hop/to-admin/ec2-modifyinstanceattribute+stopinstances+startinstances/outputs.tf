...
output "attack_path" {
  ...
  value       = "User (pl-prod-mia-to-admin-starting-user) → (ec2:StopInstances) → (ec2:ModifyInstanceAttribute with malicious cloud-init payload) → (ec2:StartInstances) → malicious script executes on boot → extract credentials from IMDS at 169.254.169.254 → admin access via pl-prod-mia-to-admin-target-role"
}
