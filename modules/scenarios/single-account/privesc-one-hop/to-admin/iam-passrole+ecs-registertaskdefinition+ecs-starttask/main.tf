...
# Resource naming convention: pl-prod-precsst-to-admin-{resource-type}
# precsst = PassRole+ECS:RegisterTaskDefinition+ECS:StartTask
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-precsst-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_required" {
  provider = aws.prod
  name     = "pl-prod-precsst-to-admin-required-permissions"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_user_policy" "starting_user_helpful" {
  provider = aws.prod
  name     = "pl-prod-precsst-to-admin-helpful-permissions"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-precsst-to-admin-target-role"
...
resource "aws_security_group" "container_instance" {
  provider    = aws.prod
  name        = "pl-prod-precsst-to-admin-sg"
...
resource "aws_iam_role" "container_instance" {
  provider = aws.prod
  name     = "pl-prod-precsst-to-admin-instance-role"
...
resource "aws_iam_instance_profile" "container_instance" {
  provider = aws.prod
  name     = "pl-prod-precsst-to-admin-instance-profile"
  role     = aws_iam_role.container_instance.name
...
resource "aws_instance" "container_instance" {
  ...
  tags = {
    Name        = "pl-prod-precsst-to-admin-instance"
    Environment = var.environment
    Scenario    = "iam-passrole+ecs-registertaskdefinition+ecs-starttask"
