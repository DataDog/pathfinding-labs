...
# Resource naming convention: pl-prod-aus-to-admin-{resource-type}
# All resources use provider = aws.prod

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-aus-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-aus-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:apprunner:*:${var.account_id}:service/pl-prod-aus-to-admin-target-service/*"
      },
      {
        Effect = "Allow"
        Action = [
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-aus-to-admin-target-role"
...
resource "aws_iam_role_policy" "target_role_admin_policy" {
  provider = aws.prod
  name     = "pl-prod-aus-to-admin-target-role-policy"
...
resource "aws_apprunner_service" "target_service" {
  provider     = aws.prod
  service_name = "pl-prod-aus-to-admin-target-service"
  ...
  tags = {
    Name        = "pl-prod-aus-to-admin-target-service"
    Environment = var.environment
    Scenario    = "apprunner-updateservice"
