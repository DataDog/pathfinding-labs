...
if [ -z "$MODULE_OUTPUT" ]; then
    echo -e "${RED}Error: Could not find OTEL_TRACES_EXPORTER= terraform output${NC}"
...
if [ "$STARTING_ACCESS_KEY_ID" == "null" ] || [ -z "$STARTING_ACCESS_KEY_ID" ]; then
    echo -e "${RED}Error: Could not extract credentials from OTEL_TRACES_EXPORTER= terraform output${NC}"
    exit 1
fi
...
echo "STARTING_USER=pl-prod-iam-006-to-admin-starting-user"
echo "ADMIN_USER=pl-prod-iam-006-to-admin-target-user"
echo "ADMIN_USER_ARN=arn:aws:iam::${ACCOUNT_ID}:user/pl-prod-iam-006-to-admin-target-user"
