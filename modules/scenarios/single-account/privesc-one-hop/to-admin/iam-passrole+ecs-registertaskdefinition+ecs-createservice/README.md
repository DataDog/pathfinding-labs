...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-precscs-to-admin-starting-user` (Scenario-specific starting user with PassRole and ECS permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-precscs-to-admin-target-role` (Admin role passed to ECS service for task execution)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-precscs-to-admin-starting-user] -->|ecs:RegisterTaskDefinition with admin role| B[ECS Task Definition]
    B -->|ecs:CreateService| C[ECS Service]
    C -->|Launches persistent task| D[ECS Fargate Task]
    D -->|Executes with| E[pl-prod-precscs-to-admin-target-role]
    E -->|iam:AttachUserPolicy| F[pl-prod-precscs-to-admin-starting-user + AdministratorAccess]
...
1. **Initial Access**: Start as `pl-prod-precscs-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-precscs-to-admin-starting-user` | Scenario-specific starting user with access keys and ECS permissions |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-precscs-to-admin-target-role` | Admin role that can be passed to ECS services (trusts ecs-tasks.amazonaws.com) |
| `arn:aws:ecs:REGION:PROD_ACCOUNT:cluster/pl-prod-precscs-cluster` | ECS cluster for running Fargate services |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole+ecs-registertaskdefinition+ecs-createservice
./demo_attack.sh
```
