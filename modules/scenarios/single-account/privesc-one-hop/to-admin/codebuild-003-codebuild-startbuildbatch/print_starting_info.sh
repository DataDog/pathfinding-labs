...
echo "STARTING_USER=pl-prod-codebuild-003-to-admin-starting-user"
echo "TARGET_ROLE=pl-prod-codebuild-003-to-admin-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-codebuild-003-to-admin-target-role"

echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
echo "EXISTING_CODEBUILD_PROJECT_NAME=pl-prod-codebuild-003-to-admin-target-project"
echo "EXISTING_CODEBUILD_PROJECT_ARN=arn:aws:codebuild:${AWS_REGION}:${ACCOUNT_ID}:project/pl-prod-codebuild-003-to-admin-target-project"
