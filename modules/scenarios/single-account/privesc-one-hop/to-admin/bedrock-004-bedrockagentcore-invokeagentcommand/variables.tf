...
variable "prod_account_aws_profile" {
  description = "AWS profile name for the prod account (used when building and pushing the container image to prod ECR). Defaults to empty string which uses ambient credentials."
  type        = string
  default     = ""
}
