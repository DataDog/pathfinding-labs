...
# [OBSERVATION] Step 5: Check target role's current inline policies
...
# [EXPLOIT] Step 6: Add inline admin policy to target role using PutRolePolicy
...
# [OBSERVATION] Step 7: Verify inline policy was added
...
# The assumed target-role credentials carry full AdministratorAccess via the inline
# policy attached in Step 6. Use those credentials to read the scenario flag from
# SSM Parameter Store.
echo -e "${YELLOW}Step 10: Capturing CTF flag from SSM Parameter Store${NC}"
FLAG_PARAM_NAME="/pathfinding-labs/flags/iam-017-to-admin"
...
echo "  $STARTING_USER → (PutRolePolicy) → $TARGET_ROLE → (AssumeRole) → Admin → (ssm:GetParameter) → CTF Flag"
