...
echo "STARTING_USER=pl-prod-lufclif-to-admin-starting-user"
echo "TARGET_ROLE=pl-prod-lufclif-to-admin-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-lufclif-to-admin-target-role"

echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
echo "EXISTING_LAMBDA_FUNCTION_NAME=pl-prod-lufclif-to-admin-target-lambda"
echo "EXISTING_LAMBDA_FUNCTION_ARN=arn:aws:lambda:${AWS_REGION}:${ACCOUNT_ID}:function/pl-prod-lufclif-to-admin-target-lambda"
