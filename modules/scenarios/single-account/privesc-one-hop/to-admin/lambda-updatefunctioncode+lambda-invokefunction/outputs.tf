...
output "attack_path" {
  ...
  value       = "User (pl-prod-lufclif-to-admin-starting-user) → UpdateFunctionCode → Lambda Function (pl-prod-lufclif-to-admin-target-lambda) → InvokeFunction (manual trigger) → Executes with Target Role (pl-prod-lufclif-to-admin-target-role) → Admin Access"
}
