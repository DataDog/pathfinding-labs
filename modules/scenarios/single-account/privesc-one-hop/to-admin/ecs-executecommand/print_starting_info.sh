...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_ecs_executecommand.value // empty')
...
echo "STARTING_USER=pl-prod-eec-to-admin-starting-user"
echo "TARGET_ROLE=pl-prod-eec-to-admin-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-eec-to-admin-target-role"
echo "ECS_CLUSTER=pl-prod-eec-to-admin-cluster"
echo "ECS_SERVICE=pl-prod-eec-to-admin-service"
echo "CONTAINER_NAME=sleep-container"
