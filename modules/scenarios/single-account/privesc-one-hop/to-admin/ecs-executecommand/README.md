...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-eec-to-admin-starting-user` (Scenario-specific starting user with ecs:ExecuteCommand permission)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-eec-to-admin-target-role` (Admin task role attached to the running ECS task)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-eec-to-admin-starting-user] -->|ecs:ExecuteCommand| B[ECS Task Container]
    B -->|curl metadata service| C[Task Role Credentials]
    C -->|Use credentials| D[pl-prod-eec-to-admin-target-role]
...
1. **Initial Access**: Start as `pl-prod-eec-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-eec-to-admin-starting-user` | Scenario-specific starting user with access keys and ecs:ExecuteCommand permission |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-eec-to-admin-target-role` | Admin task role with AdministratorAccess attached to the running ECS task |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-eec-to-admin-execution-role` | ECS execution role for pulling images and CloudWatch logging |
| `arn:aws:ecs:REGION:PROD_ACCOUNT:cluster/pl-prod-eec-to-admin-cluster` | ECS cluster hosting the vulnerable task |
| `arn:aws:ecs:REGION:PROD_ACCOUNT:service/pl-prod-eec-to-admin-cluster/pl-prod-eec-to-admin-service` | ECS service that maintains the running task with ECS Exec enabled |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/ecs-executecommand
./demo_attack.sh
```
...
aws ecs list-tasks --cluster pl-prod-eec-to-admin-cluster

# 3. Get task details (to find the container name)
aws ecs describe-tasks --cluster pl-prod-eec-to-admin-cluster --tasks <task-arn>

# 4. Execute command to shell into the container
aws ecs execute-command \
    --cluster pl-prod-eec-to-admin-cluster \
    --task <task-arn> \
