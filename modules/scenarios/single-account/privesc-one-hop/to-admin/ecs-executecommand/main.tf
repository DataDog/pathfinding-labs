...
# Resource naming convention: pl-prod-eec-to-admin-{resource-type}
# eec = ECS ExecuteCommand
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-eec-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_required" {
  provider = aws.prod
  name     = "pl-prod-eec-to-admin-required-permissions"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_user_policy" "starting_user_helpful" {
  provider = aws.prod
  name     = "pl-prod-eec-to-admin-helpful-permissions"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-eec-to-admin-target-role"
...
resource "aws_iam_role_policy" "target_role_ssm" {
  provider = aws.prod
  name     = "pl-prod-eec-to-admin-ssm-permissions"
...
resource "aws_iam_role" "execution_role" {
  provider = aws.prod
  name     = "pl-prod-eec-to-admin-execution-role"
...
resource "aws_ecs_cluster" "cluster" {
  provider = aws.prod
  name     = "pl-prod-eec-to-admin-cluster"
...
resource "aws_cloudwatch_log_group" "ecs" {
  provider          = aws.prod
  name              = "/ecs/pl-prod-eec-to-admin"
  retention_in_days = 7

  tags = {
    Name        = "pl-prod-eec-to-admin-logs"
    Environment = var.environment
    Scenario    = "ecs-executecommand"
...
resource "aws_ecs_task_definition" "task" {
  provider                 = aws.prod
  family                   = "pl-prod-eec-to-admin-task"
  network_mode             = "awsvpc"
  ...
  tags = {
    Name        = "pl-prod-eec-to-admin-task"
    Environment = var.environment
    Scenario    = "ecs-executecommand"
...
resource "aws_ecs_service" "service" {
  provider        = aws.prod
  name            = "pl-prod-eec-to-admin-service"
