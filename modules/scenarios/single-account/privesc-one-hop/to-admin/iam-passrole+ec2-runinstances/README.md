...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prec-to-admin-starting-user` (Starting user with PassRole + RunInstances permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prec-to-admin-target-role` (Admin role used by EC2 instance to attach policy)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-prec-to-admin-starting-user] -->|iam:PassRole + ec2:RunInstances| B[EC2 Instance with Admin Role]
    B -->|iam:AttachUserPolicy| C[pl-prod-prec-to-admin-starting-user + AdministratorAccess]
...
1. **Initial Access**: Start as `pl-prod-prec-to-admin-starting-user` with PassRole and RunInstances permissions (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prec-to-admin-starting-user` | Starting user with PassRole and RunInstances permissions (with access keys) |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prec-to-admin-target-role` | Admin role that EC2 instance uses to attach policy (trusts ec2.amazonaws.com) |
| `arn:aws:iam::PROD_ACCOUNT:instance-profile/pl-prod-prec-to-admin-instance-profile` | Instance profile wrapping the admin role |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole+ec2-runinstances
./demo_attack.sh
```
...
After demonstrating the attack, clean up the EC2 instance and detach the AdministratorAccess policy from the starting user:
