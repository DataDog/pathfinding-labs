...
# Pre-created Glue job outputs
output "glue_job_name" {
  description = "Name of the pre-created Glue job"
  value       = aws_glue_job.pre_created_job.name
}
...
output "benign_script_s3_path" {
  description = "S3 path to the benign script (initial job configuration)"
...
output "attack_path" {
  ...
  value       = "User (pl-prod-glue-006-to-admin-starting-user) → glue:UpdateJob → Update existing job to use admin role and malicious script → glue:CreateTrigger (--start-on-creation) → Job executes and attaches AdministratorAccess to starting user → Admin access"
}
