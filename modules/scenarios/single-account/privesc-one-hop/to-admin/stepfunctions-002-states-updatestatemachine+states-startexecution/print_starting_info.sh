...
echo "STARTING_USER=pl-prod-stepfunctions-002-to-admin-starting-user"
echo "STATE_MACHINE_ARN=$STATE_MACHINE_ARN"
echo "STATE_MACHINE_ROLE=pl-prod-stepfunctions-002-to-admin-statemachine-role"
echo "FLAG_PARAM_NAME=/pathfinding-labs/flags/stepfunctions-002-to-admin"

echo ""

echo -e "${BLUE}Attack Path:${NC}"
echo "pl-prod-stepfunctions-002-to-admin-starting-user"
echo "  → states:UpdateStateMachine (replace benign ASL with malicious definition)"
echo "  → states:StartExecution (run as pre-existing admin role)"
echo "  → iam:AttachUserPolicy AdministratorAccess on starting user"
echo "  → ssm:GetParameter /pathfinding-labs/flags/stepfunctions-002-to-admin"
echo ""
