...
output "attack_path" {
  ...
  value       = "User (${aws_iam_user.starting_user.name}) → (ecs:CreateCluster) → (ecs:RegisterTaskDefinition with admin role) → (ecs:RunTask on Fargate) → ECS task attaches admin policy to starting user → admin access"
}
