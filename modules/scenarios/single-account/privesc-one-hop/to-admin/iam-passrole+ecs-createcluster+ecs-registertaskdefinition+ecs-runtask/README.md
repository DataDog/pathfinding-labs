...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-preccrt-to-admin-starting-user` (Scenario-specific starting user with ECS and PassRole permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-preccrt-to-admin-target-role` (Privileged role passed to the ECS task)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-preccrt-to-admin-starting-user] -->|ecs:CreateCluster| B[New ECS Cluster]
    A -->|ecs:RegisterTaskDefinition<br/>with target role| C[Malicious Task Definition]
    A -->|ecs:RunTask| D[ECS Fargate Task]
    D -->|Executes with| E[pl-prod-preccrt-to-admin-target-role]
...
1. **Initial Access**: Start as `pl-prod-preccrt-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-preccrt-to-admin-starting-user` | Scenario-specific starting user with access keys, ECS cluster creation, task definition registration, and task execution permissions |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-preccrt-to-admin-target-role` | Privileged role with administrative permissions that can be passed to ECS tasks |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole+ecs-createcluster+ecs-registertaskdefinition+ecs-runtask
./demo_attack.sh
```
