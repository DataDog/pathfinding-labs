...
output "attack_path" {
  ...
  value       = "starting_user (${aws_iam_user.starting_user.name}) → (ecs:RegisterTaskDefinition with admin role) → (ecs:RunTask) → ECS task attaches admin policy to starting user → admin access"
}
