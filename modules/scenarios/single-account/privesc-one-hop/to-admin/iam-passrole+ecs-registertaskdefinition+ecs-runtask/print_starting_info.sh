...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_iam_passrole_ecs_registertaskdefinition_ecs_runtask.value // empty')
...
echo "STARTING_USER=pl-prod-precsrt-to-admin-starting-user"
echo "ADMIN_ROLE=pl-prod-precsrt-to-admin-target-role"
echo "ADMIN_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-precsrt-to-admin-target-role"
echo "TASK_EXECUTION_ROLE=pl-prod-precsrt-to-admin-target-role"
echo "TASK_EXECUTION_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-precsrt-to-admin-target-role"
