...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_iam_passrole_lambda_createfunction_lambda_addpermission.value // empty')
...
echo "STARTING_USER=pl-prod-iprlcflap-to-admin-starting-user"
echo "ADMIN_ROLE=pl-prod-iprlcflap-to-admin-target-role"
echo "ADMIN_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-iprlcflap-to-admin-target-role"
echo "LAMBDA_FUNCTION_NAME=pl-iprlcflap-malicious-function"
echo "LAMBDA_FUNCTION_NAME_ARN=arn:aws:lambda:${AWS_REGION}:${ACCOUNT_ID}:function/pl-iprlcflap-malicious-function"
