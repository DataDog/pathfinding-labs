...
# Resource naming convention: pl-prod-guj-sjr-to-admin-{resource-type}
# Provider: aws.prod

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-guj-sjr-to-admin-starting-user"
...
resource "aws_s3_bucket" "script_bucket" {
  provider = aws.prod
  bucket   = "pl-glue-scripts-guj-sjr-${var.account_id}-${var.resource_suffix}"

  tags = {
    Name        = "pl-glue-scripts-guj-sjr-${var.account_id}-${var.resource_suffix}"
    Environment = var.environment
    Scenario    = "iam-passrole+glue-updatejob+glue-startjobrun"
...
resource "aws_iam_role" "initial_role" {
  provider = aws.prod
  name     = "pl-prod-guj-sjr-to-admin-initial-role"
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-guj-sjr-to-admin-target-role"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-guj-sjr-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
