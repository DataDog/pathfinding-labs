...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_iam_passrole_lambda_createfunction_createeventsourcemapping_dynamodb.value // empty')
...
echo "STARTING_USER=pl-prod-prcfcesmd-to-admin-starting-user"
echo "TARGET_ROLE=pl-prod-prcfcesmd-to-admin-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-prcfcesmd-to-admin-target-role"
echo "LAMBDA_FUNCTION_NAME=pl-prod-prcfcesmd-malicious-lambda"
echo "LAMBDA_FUNCTION_NAME_ARN=arn:aws:lambda:${AWS_REGION}:${ACCOUNT_ID}:function/pl-prod-prcfcesmd-malicious-lambda"
