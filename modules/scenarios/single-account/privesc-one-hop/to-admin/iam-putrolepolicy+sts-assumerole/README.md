...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prpsar-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prpsar-to-admin-target-role` (Target role that will be modified and assumed)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-prpsar-to-admin-starting-user] -->|iam:PutRolePolicy| B[pl-prod-prpsar-to-admin-target-role]
    B -->|Add inline admin policy| C[pl-prod-prpsar-to-admin-target-role with admin access]
    A -->|sts:AssumeRole| C
...
1. **Initial Access**: Start as `pl-prod-prpsar-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-prpsar-to-admin-starting-user` | Scenario-specific starting user with access keys and inline policy |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-prpsar-to-admin-target-role` | Target role that trusts the starting user and can be modified |
