...
# iam:PassRole + codebuild:CreateProject + codebuild:StartBuild privilege escalation scenario
#
# This scenario demonstrates how an attacker with codebuild:CreateProject, codebuild:StartBuild,
...
# Resource naming convention: pl-prod-codebuild-001-to-admin-{resource-type}
# Provider: aws.prod (single account scenario)

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-codebuild-001-to-admin-starting-user"

  tags = {
    Name        = "pl-prod-codebuild-001-to-admin-starting-user"
    Environment = var.environment
    Scenario    = "iam-passrole+codebuild-createproject+codebuild-startbuild"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-codebuild-001-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-codebuild-001-to-admin-target-role"
...
resource "aws_ssm_parameter" "flag" {
  ...
  description = "CTF flag for the codebuild-001 to-admin scenario"
  type        = "String"
  value       = var.flag_value

  tags = {
    Name        = "pl-prod-codebuild-001-to-admin-flag"
    Environment = var.environment
    Scenario    = "iam-passrole+codebuild-createproject+codebuild-startbuild"
    Purpose     = "ctf-flag"
  }
...
resource "aws_iam_role_policy" "target_role_policy" {
  provider = aws.prod
  name     = "pl-prod-codebuild-001-to-admin-target-role-policy"
