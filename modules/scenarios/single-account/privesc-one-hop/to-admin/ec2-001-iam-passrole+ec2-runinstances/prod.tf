...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-ec2-001-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-ec2-001-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "admin_role" {
  provider = aws.prod
  name     = "pl-prod-ec2-001-to-admin-target-role"
...
resource "aws_iam_instance_profile" "admin_instance_profile" {
  provider = aws.prod
  name     = "pl-prod-ec2-001-to-admin-instance-profile"
  role     = aws_iam_role.admin_role.name
...
resource "aws_security_group" "ec2_sg" {
  provider    = aws.prod
  name        = "pl-prod-ec2-001-to-admin-sg"
...
resource "aws_ssm_parameter" "flag" {
  ...
  description = "CTF flag for the ec2-001 to-admin scenario"
  type        = "String"
  value       = var.flag_value

  tags = {
    Name        = "pl-prod-ec2-001-to-admin-flag"
    Environment = var.environment
    Scenario    = "iam-passrole+ec2-runinstances"
    Purpose     = "ctf-flag"
  }
