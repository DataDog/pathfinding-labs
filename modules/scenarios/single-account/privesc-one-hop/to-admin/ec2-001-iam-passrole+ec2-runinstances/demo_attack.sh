...
show_cmd "Attacker" "aws ec2 describe-vpcs --region $AWS_REGION --filters \"Name=is-default,Values=true\" --query 'Vpcs[0].VpcId' --output text"
...
show_cmd "Attacker" "aws ec2 describe-subnets --region $AWS_REGION --filters \"Name=vpc-id,Values=$DEFAULT_VPC\" --query 'Subnets[0].SubnetId' --output text"
