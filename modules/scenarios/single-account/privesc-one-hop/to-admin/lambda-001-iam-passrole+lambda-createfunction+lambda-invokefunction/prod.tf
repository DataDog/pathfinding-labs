...
# Resource naming convention: pl-prod-lambda-001-to-admin-{resource-type}
# lambda-001 = pathfinding.cloud ID for PassRole + Lambda CreateFunction + Lambda InvokeFunction
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-lambda-001-to-admin-starting-user"
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-lambda-001-to-admin-target-role"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-lambda-001-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
