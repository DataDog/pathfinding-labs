...
echo "STARTING_USER=pl-prod-lambda-004-to-admin-starting-user"
echo "TARGET_ROLE=pl-prod-lambda-004-to-admin-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-lambda-004-to-admin-target-role"

echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
echo "EXISTING_LAMBDA_FUNCTION_NAME=pl-prod-lambda-004-to-admin-target-lambda"
echo "EXISTING_LAMBDA_FUNCTION_ARN=arn:aws:lambda:${AWS_REGION}:${ACCOUNT_ID}:function/pl-prod-lambda-004-to-admin-target-lambda"
