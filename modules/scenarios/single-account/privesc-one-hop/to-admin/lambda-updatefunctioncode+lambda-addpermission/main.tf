...
# Resource naming convention: pl-prod-lufclap-to-admin-{resource-type}
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-lufclap-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-lufclap-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:lambda:${data.aws_region.current.name}:${var.account_id}:function:pl-prod-lufclap-to-admin-target-lambda"
      },
      {
...
resource "aws_iam_role" "lambda_exec_role" {
  provider = aws.prod
  name     = "pl-prod-lufclap-to-admin-lambda-exec-role"
...
resource "aws_lambda_function" "target_lambda" {
  ...
  function_name    = "pl-prod-lufclap-to-admin-target-lambda"
  ...
  tags = {
    Name        = "pl-prod-lufclap-to-admin-target-lambda"
    Environment = var.environment
    Scenario    = "lambda-updatefunctioncode+lambda-addpermission"
