...
output "attack_path" {
  ...
  value       = "User (pl-prod-lufclap-to-admin-starting-user) → lambda:UpdateFunctionCode → target Lambda → lambda:AddPermission → allow self invoke → lambda:InvokeFunction → execute as lambda-exec-role (AdministratorAccess) → admin access"
}
