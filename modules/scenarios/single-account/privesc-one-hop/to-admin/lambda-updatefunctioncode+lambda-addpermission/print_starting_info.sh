...
echo "STARTING_USER=pl-prod-lufclap-to-admin-starting-user"
echo "LAMBDA_EXEC_ROLE=pl-prod-lufclap-to-admin-lambda-exec-role"
echo "LAMBDA_EXEC_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-lufclap-to-admin-lambda-exec-role"

echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
echo "EXISTING_LAMBDA_FUNCTION_NAME=pl-prod-lufclap-to-admin-target-lambda"
echo "EXISTING_LAMBDA_FUNCTION_ARN=arn:aws:lambda:${AWS_REGION}:${ACCOUNT_ID}:function/pl-prod-lufclap-to-admin-target-lambda"
