...
echo "STARTING_USER=pl-prod-apprunner-002-to-admin-starting-user"
echo "TARGET_ROLE=pl-prod-apprunner-002-to-admin-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-apprunner-002-to-admin-target-role"

echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
# App Runner service ARN is more complex, just print the service name
echo "EXISTING_APPRUNNER_SERVICE_NAME=pl-prod-apprunner-002-to-admin-target-service"
echo "EXISTING_APPRUNNER_SERVICE_ARN=arn:aws:apprunner:${AWS_REGION}:${ACCOUNT_ID}:service/pl-prod-apprunner-002-to-admin-target-service/*"
