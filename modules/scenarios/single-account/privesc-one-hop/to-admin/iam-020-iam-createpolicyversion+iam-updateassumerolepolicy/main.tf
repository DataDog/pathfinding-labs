...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-iam-020-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-iam-020-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_policy" "target_policy" {
  provider    = aws.prod
  name        = "pl-prod-iam-020-to-admin-target-policy"
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-iam-020-to-admin-target-role"
