...
# Resource naming convention: pl-prod-cus-to-admin-{resource-type}
# All resources use provider = aws.prod

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-cus-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-cus-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:cloudformation:*:${var.account_id}:stack/pl-prod-cus-to-admin-stack/*"
      },
      {
      ...
      {
        Sid    = "verifyRoleCreation"
        Effect = "Allow"
        Action = [
          "iam:GetRole"
        ]
        Resource = "arn:aws:iam::${var.account_id}:role/pl-prod-cus-to-admin-escalated-role"
      },
      {
...
resource "aws_iam_role" "stack_role" {
  provider = aws.prod
  name     = "pl-prod-cus-to-admin-stack-role"
...
resource "aws_cloudformation_stack" "vulnerable_stack" {
  provider = aws.prod
  name     = "pl-prod-cus-to-admin-stack"
  ...
  template_body = jsonencode({
    ...
    Resources = {
      InitialBucket = {
        Type = "AWS::S3::Bucket"
        Properties = {
          BucketName = "pl-prod-cus-to-admin-initial-bucket-${var.account_id}-${var.resource_suffix}"
          Tags = [
            {
              Key   = "Name"
