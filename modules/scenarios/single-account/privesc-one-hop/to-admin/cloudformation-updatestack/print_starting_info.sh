...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_cloudformation_updatestack.value // empty')
...
echo "STARTING_USER=pl-prod-cus-to-admin-starting-user"
echo "ESCALATED_ROLE_NAME=pl-prod-cus-to-admin-escalated-role"
echo "ESCALATED_ROLE_NAME_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-cus-to-admin-escalated-role"

echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
echo "EXISTING_STACK_NAME=pl-prod-cus-to-admin-stack"
echo "EXISTING_STACK_ARN=arn:aws:cloudformation:${AWS_REGION}:${ACCOUNT_ID}:stack/pl-prod-cus-to-admin-stack/*"
