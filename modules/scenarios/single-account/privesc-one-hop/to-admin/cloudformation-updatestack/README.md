...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cus-to-admin-starting-user` (Scenario-specific starting user with limited permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cus-to-admin-stack-role` (CloudFormation service role with administrative permissions)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cus-to-admin-escalated-role` (Admin role created by stack update)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-cus-to-admin-starting-user] -->|cloudformation:UpdateStack| B[CloudFormation Stack]
    B -->|Executes with| C[pl-prod-cus-to-admin-stack-role]
    C -->|iam:CreateRole| D[pl-prod-cus-to-admin-escalated-role]
    A -->|sts:AssumeRole| D
...
1. **Initial Access**: Start as `pl-prod-cus-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cus-to-admin-starting-user` | Scenario-specific starting user with access keys and cloudformation:UpdateStack permission |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cus-to-admin-stack-role` | CloudFormation service role with AdministratorAccess used by the stack |
| `arn:aws:cloudformation:*:PROD_ACCOUNT:stack/pl-prod-cus-to-admin-stack/*` | CloudFormation stack that can be updated by the starting user |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cus-to-admin-escalated-role` | Admin role created during stack update (created by demo script) |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/cloudformation-updatestack
./demo_attack.sh
```
