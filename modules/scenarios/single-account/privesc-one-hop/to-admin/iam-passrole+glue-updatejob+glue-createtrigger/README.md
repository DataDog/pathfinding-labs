...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-guj-ct-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:glue::PROD_ACCOUNT:job/pl-guj-ct-to-admin-job` (Pre-existing Glue job that will be modified)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-guj-ct-to-admin-target-role` (Admin role passed to Glue job during update)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-guj-ct-to-admin-starting-user] -->|glue:UpdateJob| B[Existing Glue Job]
...
1. **Initial Access**: Start as `pl-prod-guj-ct-to-admin-starting-user` (credentials provided via Terraform outputs)
2. **Identify Existing Job**: Locate the pre-existing Glue job `pl-guj-ct-to-admin-job` (created by Terraform with benign configuration)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-guj-ct-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-guj-ct-to-admin-initial-role` | Initial non-privileged role that the job starts with |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-guj-ct-to-admin-target-role` | Administrative role passed to Glue job during update |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-guj-ct-to-admin-passrole-policy` | Policy allowing PassRole on target role, glue:UpdateJob, and glue:CreateTrigger |
| `arn:aws:glue:REGION:PROD_ACCOUNT:job/pl-guj-ct-to-admin-job` | Pre-existing Glue job that will be modified during attack |
| `arn:aws:s3:::pl-glue-scripts-guj-ct-PROD_ACCOUNT-SUFFIX` | S3 bucket containing benign and malicious scripts |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole+glue-updatejob+glue-createtrigger
./demo_attack.sh
```
