...
output "attack_path" {
  ...
  value       = "User (pl-prod-guj-ct-to-admin-starting-user) → glue:UpdateJob → Update existing job to use admin role and malicious script → glue:CreateTrigger (--start-on-creation) → Job executes and attaches AdministratorAccess to starting user → Admin access"
}
