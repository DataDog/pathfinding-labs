...
# Resource naming convention: pl-prod-guj-ct-to-admin-{resource-type}
# All resources use provider = aws.prod

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-guj-ct-to-admin-starting-user"
...
resource "aws_s3_bucket" "script_bucket" {
  provider = aws.prod
  bucket   = "pl-glue-scripts-guj-ct-${var.account_id}-${var.resource_suffix}"

  tags = {
    Name        = "pl-glue-scripts-guj-ct-${var.account_id}-${var.resource_suffix}"
    Environment = var.environment
    Scenario    = "iam-passrole+glue-updatejob+glue-createtrigger"
...
resource "aws_iam_role" "initial_role" {
  provider = aws.prod
  name     = "pl-prod-guj-ct-to-admin-initial-role"
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-guj-ct-to-admin-target-role"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-guj-ct-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
