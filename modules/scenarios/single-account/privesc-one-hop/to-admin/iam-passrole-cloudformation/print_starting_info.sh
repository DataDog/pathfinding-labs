...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_iam_passrole_cloudformation.value // empty')
...
echo "STARTING_USER=pl-prod-pcf-starting-user"
echo "ADMIN_ROLE=pl-prod-pcf-admin-role"
echo "ADMIN_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-pcf-admin-role"
echo "ESCALATED_ROLE_NAME=pl-prod-pcf-escalated-role"
echo "ESCALATED_ROLE_NAME_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-pcf-escalated-role"
