...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-gcd-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-gcd-to-admin-target-role` (Admin role passed to Glue dev endpoint)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-gcd-to-admin-starting-user] -->|iam:PassRole + glue:CreateDevEndpoint| B[Glue Dev Endpoint]
    B -->|Assigned Role| C[pl-prod-gcd-to-admin-target-role]
...
1. **Initial Access**: Start as `pl-prod-gcd-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-gcd-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-gcd-to-admin-target-role` | Administrative role passed to Glue dev endpoint |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-gcd-to-admin-passrole-policy` | Policy allowing PassRole on target role and glue:CreateDevEndpoint |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/iam-passrole+glue-createdevendpoint
./demo_attack.sh
```
