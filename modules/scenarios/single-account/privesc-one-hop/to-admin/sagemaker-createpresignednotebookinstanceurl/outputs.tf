...
output "attack_path" {
  ...
  value       = "User (${aws_iam_user.starting_user.name}) → CreatePresignedNotebookInstanceUrl → Existing notebook (${aws_sagemaker_notebook_instance.target_notebook.name}) with admin role (${aws_iam_role.notebook_execution_role.name}) → Access Jupyter terminal via presigned URL → Execute commands with admin privileges → Admin access"
}
