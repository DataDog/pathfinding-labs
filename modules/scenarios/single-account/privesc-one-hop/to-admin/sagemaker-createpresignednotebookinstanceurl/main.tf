...
# Scenario shorthand: cpnu (CreatePresignedNotebookInstanceUrl)

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-${var.environment}-cpnu-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-${var.environment}-cpnu-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "notebook_execution_role" {
  provider = aws.prod
  name     = "pl-${var.environment}-cpnu-to-admin-notebook-role"
...
resource "aws_sagemaker_notebook_instance" "target_notebook" {
  provider              = aws.prod
  name                  = "pl-${var.environment}-cpnu-to-admin-notebook"
