...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-prrsi-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-prrsi-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "admin_role" {
  provider = aws.prod
  name     = "pl-prod-prrsi-to-admin-target-role"
...
resource "aws_iam_instance_profile" "admin_instance_profile" {
  provider = aws.prod
  name     = "pl-prod-prrsi-to-admin-instance-profile"
  role     = aws_iam_role.admin_role.name
...
resource "aws_security_group" "ec2_sg" {
  provider    = aws.prod
  name        = "pl-prod-prrsi-to-admin-sg"
...
# #!/bin/bash
# STARTING_USER_NAME="pl-prod-prrsi-to-admin-starting-user"
#
