...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cbsb-to-admin-starting-user` (Scenario-specific starting user)
- `arn:aws:codebuild:{region}:PROD_ACCOUNT:project/pl-prod-cbsb-to-admin-existing-project` (Pre-existing CodeBuild project)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cbsb-to-admin-project-role` (Privileged role attached to CodeBuild project)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-cbsb-to-admin-starting-user] -->|codebuild:StartBuild with buildspec-override| B[pl-prod-cbsb-to-admin-existing-project]
    B -->|Executes with| C[pl-prod-cbsb-to-admin-project-role]
...
1. **Initial Access**: Start as `pl-prod-cbsb-to-admin-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-cbsb-to-admin-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:codebuild:{region}:PROD_ACCOUNT:project/pl-prod-cbsb-to-admin-existing-project` | Pre-existing CodeBuild project with privileged role |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-cbsb-to-admin-project-role` | Privileged role with AdministratorAccess attached to CodeBuild project |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-cbsb-to-admin-user-policy` | Policy granting codebuild:StartBuild, codebuild:ListProjects, codebuild:BatchGetProjects, and codebuild:BatchGetBuilds |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-admin/codebuild-startbuild
./demo_attack.sh
```
