...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_admin_codebuild_startbuild.value // empty')
...
echo "STARTING_USER=pl-prod-cbsb-to-admin-starting-user"
echo "PROJECT_ROLE=pl-prod-cbsb-to-admin-project-role"
echo "PROJECT_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-cbsb-to-admin-project-role"

echo ""

echo -e "${YELLOW}Existing Resources to Exploit:${NC}"
echo "EXISTING_CODEBUILD_PROJECT_NAME=pl-prod-cbsb-to-admin-existing-project"
echo "EXISTING_CODEBUILD_PROJECT_ARN=arn:aws:codebuild:${AWS_REGION}:${ACCOUNT_ID}:project/pl-prod-cbsb-to-admin-existing-project"
