...
output "attack_path" {
  ...
  value       = "User (pl-prod-cbsb-to-admin-starting-user) → codebuild:StartBuild with buildspec-override → ${aws_codebuild_project.existing_project.name} → Buildspec grants admin to starting user → Admin Access"
}
