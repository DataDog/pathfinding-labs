...
output "attack_path" {
  ...
  value       = "User (pl-prod-sulc-to-admin-starting-user) → StopNotebookInstance → CreateLifecycleConfig → UpdateNotebookInstance → StartNotebookInstance → lifecycle script executes with notebook's admin role (pl-prod-sulc-to-admin-notebook-role) → admin access"
}
