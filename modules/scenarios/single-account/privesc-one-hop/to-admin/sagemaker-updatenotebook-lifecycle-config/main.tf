...
# Resource naming convention: pl-prod-sulc-to-admin-{resource-type}
# SULC = SageMaker Update Lifecycle Config

# Scenario-specific starting user
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-sulc-to-admin-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-sulc-to-admin-starting-user-policy"
  user     = aws_iam_user.starting_user.name
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:sagemaker:*:${var.account_id}:notebook-instance/pl-prod-sulc-to-admin-notebook"
      },
      {
...
resource "aws_iam_role" "notebook_execution_role" {
  provider = aws.prod
  name     = "pl-prod-sulc-to-admin-notebook-role"
...
resource "aws_sagemaker_notebook_instance" "target_notebook" {
  provider      = aws.prod
  name          = "pl-prod-sulc-to-admin-notebook"
