...
# Cleanup script for bedrockagentcore-startbrowsersession+cdp privilege escalation demo (bedrock-007)
# This script removes local temporary files created during the demo.
# The Custom Browser is managed by Terraform and is NOT deleted here.
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Cleanup: AgentCore Custom Browser CDP Credential Theft (bedrock-007)${NC}"
echo -e "${GREEN}========================================${NC}\n"
...
echo ""
echo -e "${BLUE}Note: The Custom Browser is a pre-existing Terraform-managed resource and remains deployed.${NC}"
echo -e "${BLUE}The stolen temporary credentials have already expired (max 1–12 hours).${NC}"
echo ""
echo -e "${GREEN}The environment has been restored to its original state.${NC}"
echo -e "${YELLOW}The infrastructure (user, role, and browser) remains deployed.${NC}"
