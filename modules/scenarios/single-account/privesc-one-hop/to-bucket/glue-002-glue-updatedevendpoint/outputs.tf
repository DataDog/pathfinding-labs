...
output "attack_path" {
  ...
  value       = "User (pl-prod-glue-002-to-bucket-starting-user) → glue:UpdateDevEndpoint (add SSH key to ${aws_glue_dev_endpoint.target_endpoint.name}) → SSH access to endpoint → Role (${aws_iam_role.target_role.name}) → Sensitive bucket (${aws_s3_bucket.sensitive_bucket.id})"
}
