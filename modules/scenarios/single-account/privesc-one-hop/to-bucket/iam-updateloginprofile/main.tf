...
# Resource naming convention: pl-ulp-bucket-*
# ulp = UpdateLoginProfile

# Target S3 bucket with sensitive data
resource "aws_s3_bucket" "target_bucket" {
  provider = aws.prod
  bucket   = "pl-ulp-sensitive-data-${var.account_id}-${var.resource_suffix}"

  tags = {
    Name        = "pl-ulp-sensitive-data-bucket"
    Environment = var.environment
    Scenario    = "iam-updateloginprofile-bucket"
...
resource "aws_iam_user_policy" "target_user_s3_policy" {
  provider = aws.prod
  name     = "pl-prod-one-hop-updateloginprofile-bucket-target-policy"
  user     = aws_iam_user.target_user.name
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-ulp-bucket-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-one-hop-updateloginprofile-bucket-starting-policy"
  user     = aws_iam_user.starting_user.name
