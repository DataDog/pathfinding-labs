...
# SSM SendCommand privilege escalation scenario (to-bucket variant)
#
# This scenario demonstrates how a user with ssm:SendCommand permission can execute
# commands on an EC2 instance that has S3 bucket access, extract the instance's
# temporary credentials from the metadata endpoint, and gain access to sensitive S3 data.

# Resource naming convention: pl-prod-ssc-to-bucket-{resource-type}
...
resource "aws_s3_bucket" "target_bucket" {
  provider = aws.prod
  bucket   = "pl-sensitive-data-ssc-${var.account_id}-${var.resource_suffix}"

  tags = {
    Name        = "pl-sensitive-data-ssc-bucket"
...
resource "aws_s3_object" "sensitive_data" {
  ...
  content  = "This is sensitive data that should only be accessible to authorized principals. If you can read this, you have successfully extracted EC2 instance credentials via SSM SendCommand."
}
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-ssc-to-bucket-starting-user"
...
resource "aws_iam_role" "ec2_bucket_role" {
  provider = aws.prod
  name     = "pl-prod-ssc-to-bucket-ec2-bucket-role"
...
resource "aws_iam_role_policy" "ec2_bucket_access" {
  provider = aws.prod
  name     = "pl-prod-ssc-to-bucket-s3-access-policy"
...
resource "aws_iam_instance_profile" "ec2_bucket" {
  provider = aws.prod
  name     = "pl-prod-ssc-to-bucket-instance-profile"
  role     = aws_iam_role.ec2_bucket_role.name
...
resource "aws_instance" "target" {
  ...
  tags = {
    Name        = "pl-prod-ssc-to-bucket-target-instance"
    Environment = var.environment
    Scenario    = "ssm-sendcommand"
