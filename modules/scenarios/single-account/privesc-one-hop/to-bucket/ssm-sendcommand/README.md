...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-ssc-to-bucket-starting-user` (Scenario-specific starting user)
- `arn:aws:ec2:REGION:PROD_ACCOUNT:instance/i-xxxxxxxxx` (EC2 instance with SSM agent)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-ssc-to-bucket-ec2-bucket-role` (S3 access role attached to EC2 instance)
- `arn:aws:s3:::pl-sensitive-data-ssc-PROD_ACCOUNT-SUFFIX` (Target sensitive S3 bucket)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-ssc-to-bucket-starting-user] -->|ssm:SendCommand| B[EC2 Instance]
...
1. **Initial Access**: Start as `pl-prod-ssc-to-bucket-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-ssc-to-bucket-starting-user` | Scenario-specific starting user with access keys and SSM permissions |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-ssc-to-bucket-ec2-bucket-role` | S3 access role attached to the EC2 instance (target for credential extraction) |
| `arn:aws:iam::PROD_ACCOUNT:instance-profile/pl-prod-ssc-to-bucket-instance-profile` | Instance profile associating the S3 role with the EC2 instance |
| `arn:aws:ec2:REGION:PROD_ACCOUNT:instance/i-xxxxxxxxx` | EC2 instance with SSM agent and S3 access role attached |
| `arn:aws:s3:::pl-sensitive-data-ssc-PROD_ACCOUNT-SUFFIX` | Target S3 bucket containing sensitive data |
| `arn:aws:s3:::pl-sensitive-data-ssc-PROD_ACCOUNT-SUFFIX/sensitive-data.txt` | Sensitive file in the target bucket |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-bucket/ssm-sendcommand
./demo_attack.sh
```
