...
output "attack_path" {
  ...
  value       = "User (pl-prod-ssc-to-bucket-starting-user) → ssm:SendCommand → EC2 Instance (${aws_instance.target.id}) → Extract Instance Role Credentials → S3 Bucket Access (${aws_s3_bucket.target_bucket.id})"
}
