...
output "attack_path" {
  ...
  value       = "User (pl-prod-iam-003-to-bucket-starting-user) → list access keys (iam:ListAccessKeys) → delete existing key (iam:DeleteAccessKey) → create new key (iam:CreateAccessKey) → target user (pl-prod-iam-003-to-bucket-target-user) → S3 bucket access"
}
