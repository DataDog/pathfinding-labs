...
# Resource naming convention: pl-{environment}-gcd-to-bucket-{resource-type}
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-gcd-to-bucket-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-gcd-to-bucket-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "target_role" {
  provider = aws.prod
  name     = "pl-prod-gcd-to-bucket-target-role"
...
resource "aws_iam_role_policy" "target_role_s3_policy" {
  provider = aws.prod
  name     = "pl-prod-gcd-to-bucket-target-s3-policy"
...
resource "aws_s3_bucket" "sensitive_bucket" {
  provider = aws.prod
  bucket   = "pl-sensitive-data-gcd-${var.account_id}-${var.resource_suffix}"

  tags = {
    Name        = "pl-sensitive-data-gcd-bucket"
    Environment = var.environment
    Scenario    = "iam-passrole+glue-createdevendpoint"
