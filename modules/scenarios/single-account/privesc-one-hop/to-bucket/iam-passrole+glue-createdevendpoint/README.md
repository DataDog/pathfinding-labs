...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-gcd-to-bucket-starting-user` (Scenario-specific starting user)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-gcd-to-bucket-target-role` (Target role with S3 bucket access)
- `arn:aws:s3:::pl-sensitive-data-gcd-PROD_ACCOUNT-SUFFIX` (Target sensitive S3 bucket)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-gcd-to-bucket-starting-user] -->|iam:PassRole + glue:CreateDevEndpoint| B[Glue Dev Endpoint]
    B -->|Assumes| C[pl-prod-gcd-to-bucket-target-role]
    C -->|SSH access + AWS CLI| D[pl-sensitive-data-gcd bucket]
...
1. **Initial Access**: Start as `pl-prod-gcd-to-bucket-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-gcd-to-bucket-starting-user` | Scenario-specific starting user with access keys |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-gcd-to-bucket-starting-policy` | Allows `iam:PassRole` and `glue:CreateDevEndpoint` |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-gcd-to-bucket-target-role` | Target role with S3 bucket read permissions |
| `arn:aws:iam::PROD_ACCOUNT:policy/pl-prod-gcd-to-bucket-target-policy` | Grants `s3:GetObject` and `s3:ListBucket` on sensitive bucket |
| `arn:aws:s3:::pl-sensitive-data-gcd-PROD_ACCOUNT-SUFFIX` | Target sensitive S3 bucket containing sample data |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-bucket/iam-passrole+glue-createdevendpoint
./demo_attack.sh
```
