...
# Cleanup script for lambda:UpdateFunctionCode to S3 bucket access demo
# This script restores the original Lambda function code and removes the injected
# TARGET_BUCKET environment variable added during the demo.
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Cleanup: Lambda UpdateFunctionCode to S3 Bucket${NC}"
echo -e "${GREEN}========================================${NC}\n"
...
if [ -f /tmp/original_lambda_backup.zip ]; then
    ...
    echo "  2. Run: terraform apply -replace='module.single_account_privesc_one_hop_to_bucket_lambda_003_lambda_updatefunctioncode[0].aws_lambda_function.target_lambda'"
fi
echo ""

# Step 3: Wait for Lambda to process the code restoration
if [ -f /tmp/original_lambda_backup.zip ]; then
    echo -e "${YELLOW}Step 3: Waiting for Lambda to process code restoration${NC}"
    echo "Allowing time for Lambda to deploy the original code..."
    sleep 10
    echo -e "${GREEN}✓ Lambda function code restored${NC}\n"
fi
...
# Verify Lambda function still exists
...
if [ $? -eq 0 ]; then
    ...
    echo "Last update status: $VERIFICATION"
else
...
echo -e "${YELLOW}The infrastructure (users, roles, Lambda function, and S3 bucket) remains deployed${NC}"
