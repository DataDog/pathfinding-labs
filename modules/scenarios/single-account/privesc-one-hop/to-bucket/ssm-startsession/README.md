...
- `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-sss-to-bucket-starting-user` (Scenario-specific starting user)
- `arn:aws:ec2:REGION:PROD_ACCOUNT:instance/i-xxxxxxxxx` (EC2 instance with SSM agent)
- `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-sss-to-bucket-ec2-role` (S3 access role attached to EC2 instance)
- `arn:aws:s3:::pl-sensitive-data-sss-to-bucket-PROD_ACCOUNT-SUFFIX` (Target sensitive S3 bucket)

### Attack Path Diagram

```mermaid
graph LR
    A[pl-prod-sss-to-bucket-starting-user] -->|ssm:StartSession| B[Interactive Shell on EC2]
...
1. **Initial Access**: Start as `pl-prod-sss-to-bucket-starting-user` (credentials provided via Terraform outputs)
...
| ARN | Purpose |
| -- | -- |
| `arn:aws:iam::PROD_ACCOUNT:user/pl-prod-sss-to-bucket-starting-user` | Scenario-specific starting user with access keys and SSM StartSession permissions |
| `arn:aws:iam::PROD_ACCOUNT:role/pl-prod-sss-to-bucket-ec2-role` | S3 access role attached to the EC2 instance (target for credential extraction) |
| `arn:aws:iam::PROD_ACCOUNT:instance-profile/pl-prod-sss-to-bucket-instance-profile` | Instance profile associating the S3 role with the EC2 instance |
| `arn:aws:ec2:REGION:PROD_ACCOUNT:instance/i-xxxxxxxxx` | EC2 instance with SSM agent and S3 access role attached |
| `arn:aws:s3:::pl-sensitive-data-sss-to-bucket-PROD_ACCOUNT-SUFFIX` | Target S3 bucket containing sensitive data |
| `arn:aws:s3:::pl-sensitive-data-sss-to-bucket-PROD_ACCOUNT-SUFFIX/sensitive-data.txt` | Sensitive file in the target bucket |
...
```bash
cd modules/scenarios/single-account/privesc-one-hop/to-bucket/ssm-startsession
./demo_attack.sh
```
