...
# Resource naming convention: pl-prod-sss-to-bucket-{resource-type}
...
resource "aws_s3_bucket" "target_bucket" {
  provider = aws.prod
  bucket   = "pl-sensitive-data-sss-${var.account_id}-${var.resource_suffix}"

  tags = {
    Name        = "pl-sensitive-data-sss-bucket"
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-sss-to-bucket-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-sss-to-bucket-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_role" "ec2_bucket_role" {
  provider = aws.prod
  name     = "pl-prod-sss-to-bucket-ec2-role"
...
resource "aws_iam_role_policy" "ec2_bucket_access" {
  provider = aws.prod
  name     = "pl-prod-sss-to-bucket-s3-access-policy"
...
resource "aws_iam_instance_profile" "ec2_bucket" {
  provider = aws.prod
  name     = "pl-prod-sss-to-bucket-instance-profile"
  role     = aws_iam_role.ec2_bucket_role.name
...
resource "aws_instance" "target" {
  ...
  tags = {
    Name        = "pl-prod-sss-to-bucket-target-instance"
    Environment = var.environment
    Scenario    = "ssm-startsession"
