...
output "attack_path" {
  ...
  value       = "User (pl-prod-sss-to-bucket-starting-user) → ssm:StartSession → EC2 Instance (${aws_instance.target.id}) → Extract Instance Role Credentials via IMDS → S3 Bucket Access (${aws_s3_bucket.target_bucket.id})"
}
