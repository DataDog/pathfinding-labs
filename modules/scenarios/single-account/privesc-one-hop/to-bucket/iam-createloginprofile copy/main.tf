...
# Resource naming convention: pl-prod-clp-bucket-{resource-type}
...
resource "aws_iam_user" "starting_user" {
  provider = aws.prod
  name     = "pl-prod-clp-bucket-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-clp-bucket-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
resource "aws_iam_user_policy" "hop1_user_policy" {
  provider = aws.prod
  name     = "pl-prod-clp-bucket-hop1-s3-policy"
...
resource "aws_s3_bucket" "sensitive_bucket" {
  provider = aws.prod
  bucket   = "pl-sensitive-data-clp-${var.account_id}-${var.resource_suffix}"
