...
MODULE_OUTPUT=$(terraform output -json 2>/dev/null | jq -r '.single_account_privesc_one_hop_to_bucket_iam_updateassumerolepolicy.value // empty')
...
echo "STARTING_USER=pl-prod-uar-to-bucket-starting-user"
echo "STARTING_ROLE=pl-prod-uar-to-bucket-starting-role"
echo "STARTING_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-uar-to-bucket-starting-role"
echo "TARGET_ROLE=pl-prod-uar-to-bucket-target-role"
echo "TARGET_ROLE_ARN=arn:aws:iam::${ACCOUNT_ID}:role/pl-prod-uar-to-bucket-target-role"
echo "STARTING_ROLE_ARN=$STARTING_ROLE_ARN"
