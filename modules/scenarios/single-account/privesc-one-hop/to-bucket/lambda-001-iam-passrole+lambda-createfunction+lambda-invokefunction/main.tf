...
# iam:PassRole + lambda:CreateFunction + lambda:InvokeFunction privilege escalation scenario (to-bucket)
#
# This scenario demonstrates how a user with iam:PassRole, lambda:CreateFunction, and
# lambda:InvokeFunction can escalate by creating a Lambda function with an S3-access role
# and invoking it to read the CTF flag from the sensitive target bucket.

# Resource naming convention: pl-prod-lambda-001-to-bucket-{resource-type}
...
resource "aws_iam_user" "starting_user" {
  provider      = aws.prod
  force_destroy = true
  name          = "pl-prod-lambda-001-to-bucket-starting-user"
...
resource "aws_iam_user_policy" "starting_user_policy" {
  provider = aws.prod
  name     = "pl-prod-lambda-001-to-bucket-starting-user-policy"
  user     = aws_iam_user.starting_user.name
...
# Target role that the attacker passes to the Lambda function (has S3 bucket access).
...
resource "aws_iam_role" "target_role" {
  ...
  name                  = "pl-prod-lambda-001-to-bucket-target-role"
...
resource "aws_iam_role_policy" "target_role_s3_policy" {
  provider = aws.prod
  name     = "pl-prod-lambda-001-to-bucket-target-s3-policy"
...
resource "aws_s3_bucket" "target_bucket" {
  provider = aws.prod
  bucket   = "pl-prod-lambda-001-to-bucket-${var.account_id}-${var.resource_suffix}"

  tags = {
    Name        = "pl-prod-lambda-001-to-bucket-target-bucket"
    Environment = var.environment
    Scenario    = "iam-passrole+lambda-createfunction+lambda-invokefunction"
...
# creating a Lambda function with the target role and invoking it to read the bucket.
...
resource "aws_s3_object" "flag" {
  ...
  tags = {
    Name        = "pl-prod-lambda-001-to-bucket-flag"
    Environment = var.environment
    Scenario    = "iam-passrole+lambda-createfunction+lambda-invokefunction"
    Purpose     = "ctf-flag"
  }
