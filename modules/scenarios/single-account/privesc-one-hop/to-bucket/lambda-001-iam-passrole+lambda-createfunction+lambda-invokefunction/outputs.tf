...
output "target_role_arn" {
  description = "ARN of the target role with S3 bucket access (passed to Lambda during exploitation)"
  value       = aws_iam_role.target_role.arn
}

output "target_role_name" {
  description = "Name of the target role with S3 bucket access"
...
output "attack_path" {
  ...
  value       = "User (pl-prod-lambda-001-to-bucket-starting-user) → iam:PassRole + lambda:CreateFunction → Lambda with target role (S3 access) → lambda:InvokeFunction → Lambda reads flag.txt from target bucket → CTF flag"
}
