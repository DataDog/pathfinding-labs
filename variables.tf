...
# Default flag values shipped with the public repo. These are the values used
# when plabs has not been configured with a custom flag file. scenario_flags
# (written by plabs) takes precedence via merge() in locals.tf.
...
variable "enable_single_account_privesc_one_hop_to_admin_batch_003_iam_passrole_batch_createcomputeenvironment_batch_createjobqueue_batch_registerjobdefinition_batch_submitjob" {
  description = "Enable: single-account → privesc-one-hop → to-admin → batch-003-iam-passrole+batch-createcomputeenvironment+batch-createjobqueue+batch-registerjobdefinition+batch-submitjob"
  type        = bool
  default     = false
}
