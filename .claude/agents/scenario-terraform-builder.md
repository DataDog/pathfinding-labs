...
- **name**: Scenario name
- **required_preconditions** (optional): Array of precondition entries. Each entry describes something that must already exist in the account for the attack to be viable (e.g., an existing Lambda function with a privileged role). **Important**: the Terraform module SHOULD create these pre-existing resources as part of its baseline setup — they represent the "vulnerable state" the attacker exploits. Do NOT skip creating them. The preconditions field documents what the Terraform establishes so that a practitioner understands what vulnerability the scenario exposes.
