...
| amplify-001 | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [ ] | **Demo + Cleanup PASS 2026-05-13** after five fixes: (1) **macOS keychain hijack** — `git config --global credential.helper '!aws codecommit credential-helper $@'` doesn't replace `osxkeychain` set in system-level gitconfig; git chains helpers, osxkeychain runs first and returns stale creds → opaque HTTP 403. Fix: per-invocation `-c "credential.helper="` (empty) prepended to clear the chain, then the codecommit helper appended, applied to both `git clone` and `git push` via a `GIT_CC_FLAGS` array. No more global-config mutation. Works on macOS/Linux/Docker (any git ≥2.9). (2) **AWS CLI v2 `--repository` preflight 401** — original failure documented at the top of this doc. Fix: pass repo + name + role via `--cli-input-json` which skips the client-side URL probe. (3) **codecommit:GetRepository required** — Amplify's CreateApp server-side validates the caller can read the repo. Added to required permissions. (4) **Amplify auto-plumbing perms** — CreateApp creates `amplify_codecommit_topic` SNS topic, subscribes Amplify, and calls `PutRepositoryTriggers` on the repo, all as the caller. Added `sns:CreateTopic` + `sns:Subscribe` (scoped to `amplify_codecommit_topic`) and `codecommit:GetRepositoryTriggers` + `codecommit:PutRepositoryTriggers` (scoped to the exploit repo) to required permissions. (5) **`set -e` on empty commit** — partial prior-run leaves `amplify.yml` + `index.html` already in the repo; rewriting them produces no diff, `git commit` exits 1, script dies. Fix: tolerate empty commit + `git push ... \|\| true`. Build ran ~75s, AdminAccess attached, flag `flag{amplify_001_admin_captured}` captured. Commit pending. |
...
3. ✅ **amplify-001 — DONE 2026-05-13.** Five fixes applied: per-invocation `-c "credential.helper="` to clear macOS keychain hijack; `--cli-input-json` to skip CLI URL preflight; `codecommit:GetRepository`, `sns:CreateTopic`/`Subscribe` on `amplify_codecommit_topic`, and `codecommit:GetRepositoryTriggers`/`PutRepositoryTriggers` added to required perms (Amplify auto-plumbs CodeCommit→SNS as the caller); `set -e`-safe empty-commit handling for replayed runs. Demo + cleanup both PASS end-to-end. Commit pending.
...
6. ✅ **omics-001 — DONE 2026-05-13.** Demo + cleanup both confirmed PASS end-to-end. The cross-account `s3:DeleteObject` bucket-policy fix applied earlier in the day held up under live cleanup. Commit pending.
